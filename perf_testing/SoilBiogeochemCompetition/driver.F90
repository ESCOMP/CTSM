program SoilBiogeochemCompetition_driver
  ...
  !
  ! Default mode (--all): runs the routine across all 8 configurations of
  ! (use_nitrif_denitrif, carbon_only, decomp_method == mimics_decomp) per
  ! iteration, summing the per-config checksums into one combined value.
  ! This locks down correctness across every top-level branch in the
  ! routine.
  !
  ...
  !
  ! Usage:
  !   ./driver [--fast] [ncol [nlevdecomp [ndct [numfc [niters]]]]]
  !
  ! Defaults: 8000 10 8 -1 1   (numfc=-1 means use ncol)
  !
  ! NOTE: the routine accumulates into some outputs (actual_immob,
  ! potential_immob in the non-nitrif branch). The driver re-zeroes all
  ! output / inout arrays before every call, so per-call checksums are
  ! independent of niters.
  !-----------------------------------------------------------------------
  ...
  integer  :: iter, nconfigs, total_calls
  real(r8) :: checksum, partial_cs
  ...
  total_calls = niters * nconfigs

  call allocate_arrays()
  call fill_inputs_once()
  ...
  per_call_s = elapsed_s / real(total_calls, r8)
#endif
...
contains
  ...
  subroutine parse_args()
    ! Accepts --fast in any position; remaining positional args are:
    !   ncol  nlevdecomp  ndct  numfc  niters
    integer :: nargs, i, pos
    ...
    do i = 1, nargs
       ...
       if (trim(arg) == '--fast') then
          is_fast = .true.
       else
          pos = pos + 1
          select case (pos)
          case (1); read(arg,*) ncol
          case (2); read(arg,*) nlevdecomp
          case (3); read(arg,*) ndct
          case (4); read(arg,*) numfc
          case (5); read(arg,*) niters
          case default
             write(*,'(a,a)') 'driver: ignoring extra arg: ', trim(arg)
          end select
       end if
    end do
  ...
  !---------------------------------------------------------------------
  subroutine fill_inputs_once()
    ! Synthetic INPUT fields (don't change between calls). zero_outputs
    ! handles the output / inout arrays before each call.
    integer  :: c, j, k, fc
    real(r8) :: frac, depthf
  ...
  end subroutine fill_inputs_once
  ...
  subroutine zero_outputs()
    ...
    ! so they must start at zero for every call.
    fpg(:)                       = 0.0_r8
    fpi(:)                       = 0.0_r8
  ...
  !---------------------------------------------------------------------
  subroutine run_config(unitrif, conly, dmethod, partial_cs)
    ! Zero outputs, call SoilBiogeochemCompetition once with the supplied
    ! per-config switches, return the post-call checksum.
    logical , intent(in)  :: unitrif, conly
    integer , intent(in)  :: dmethod
    real(r8), intent(out) :: partial_cs
    ...
    call SoilBiogeochemCompetition( &
         ...
         dt, bdnr, &
         unitrif, conly, &
         dmethod, mimics_decomp, i_cop_mic, i_oli_mic, &
    ...
    call compute_checksum(partial_cs)
  ...
  subroutine report(checksum)
    ...
    write(*,'(a,i0)')      '  configs / iter       = ', nconfigs
    write(*,'(a,i0)')      '  total calls          = ', total_calls
#ifdef PERF_TIMING
  ...
  subroutine compare_to_baseline(checksum)
    ...
    if (trim(b_mode) /= trim(mode) .or. &
        b_ncol /= ncol .or. b_nlevdecomp /= nlevdecomp .or. &
        b_ndct /= ndct .or. b_numfc /= numfc .or. &
        b_niters /= niters) then
