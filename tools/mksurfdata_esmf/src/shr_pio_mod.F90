...
#include <mpif.h>
  ...
  integer                                     :: total_comps = 1
  integer :: shr_log_unit = 6
...
!===============================================================
  ...
  subroutine shr_pio_init1(nlfilename, Global_Comm)
  ...
  !===============================================================
  subroutine shr_pio_init2(comp_comm, comp_comm_iam)

    ! If pio_async_interface is false each component namelist
    ! pio_inparm is read from compname_modelio.nml Then a subset of
    ! each components compute tasks are Identified as IO tasks using
    ! the root, stride and count variables to select the tasks.
    ...
    i = 1
    write(shr_log_unit,*) ' : pio_numiotasks = ',pio_comp_settings(i)%pio_numiotasks
    write(shr_log_unit,*) ' : pio_stride     = ',pio_comp_settings(i)%pio_stride
    write(shr_log_unit,*) ' : pio_rearranger = ',pio_comp_settings(i)%pio_rearranger
    write(shr_log_unit,*) ' : pio_root       = ',pio_comp_settings(i)%pio_root
    write(shr_log_unit,*) ' : pio_iotype     = ',pio_comp_settings(i)%pio_iotype
  ...
  subroutine shr_pio_finalize(  )
    integer :: ierr
    call pio_finalize(iosystems(1), ierr)
  end subroutine shr_pio_finalize

  !===============================================================================
  function shr_pio_getiotype() result(io_type)
    integer :: io_type
    io_type = pio_comp_settings(1)%pio_iotype
  end function shr_pio_getiotype

  !===============================================================================
  function shr_pio_getrearranger() result(io_type)
    integer :: io_type
    io_type = pio_comp_settings(1)%pio_rearranger
  end function shr_pio_getrearranger

  !===============================================================================
  function shr_pio_getioformat() result(io_format)
    integer :: io_format
    io_format = pio_comp_settings(1)%pio_netcdf_ioformat
  end function shr_pio_getioformat

  !===============================================================================
  function shr_pio_getioroot() result(io_root)
    integer :: io_root
    io_root = pio_comp_settings(1)%pio_root
  end function shr_pio_getioroot

  !===============================================================================
  function shr_pio_getiosys() result(iosystem)
    type(iosystem_desc_t), pointer :: iosystem
    iosystem => iosystems(1)
  end function shr_pio_getiosys
  ...
  subroutine shr_pio_getioformatfromname(pio_netcdf_format, pio_netcdf_ioformat, pio_default_netcdf_ioformat)
    ...
    pio_netcdf_format = shr_pio_string_toupper(pio_netcdf_format)
  ...
  subroutine shr_pio_getiotypefromname(typename, iotype, defaulttype)
    ...
    typename = shr_pio_string_toupper(typename)
  ...
  subroutine shr_pio_namelist_set(npes,mycomm, pio_stride, pio_root, pio_numiotasks, &
    ...
    call mpi_bcast(pio_iotype          , 1, MPI_INT, 0, myComm, ierr)
    call mpi_bcast(pio_stride          , 1, MPI_INT, 0, myComm, ierr)
    call mpi_bcast(pio_root            , 1, MPI_INT, 0, myComm, ierr)
    call mpi_bcast(pio_numiotasks      , 1, MPI_INT, 0, myComm, ierr)
    call mpi_bcast(pio_rearranger      , 1, MPI_INT, 0, myComm, ierr)
    call mpi_bcast(pio_netcdf_ioformat , 1, MPI_INT, 0, myComm, ierr)
  ...
  end subroutine shr_pio_rearr_opts_set

  !===============================================================================
  function shr_pio_string_toUpper(str)

    character(len=*), intent(in) :: str      ! String to convert to upper case
    character(len=len(str))      :: shr_pio_string_toUpper

    integer          :: i             ! Index
    integer          :: aseq          ! ascii collating sequence
    integer          :: LowerToUpper  ! integer to convert case
    ...
    do i = 1, len(str)
       ...
       shr_pio_string_toUpper(i:i) = ctmp
    end do
  end function shr_pio_string_toUpper
