...
! File specific settings
  ...
  integer, parameter :: nlon = 2160  !input grid : longitude points
  integer, parameter :: nlat = 1080  !input grid : latitude  points
  ...
  real(r8) :: tempelev(nlon,nlat)         !input elevation
  real(r8) :: templandmask(nlon,nlat)     !input land mask
  real(r8) :: elev(nlon,nlat)             !output elevation
  real(r8) :: landmask(nlon,nlat)         !output land mask
...
! Determine input and output file names
  ...
  filei = '/ptmp/slevis/lsminput/new/navytopo/elev.ascii'
  fileo = '/ptmp/slevis/lsminput/new/navytopo/mksrf_elev.nc'

! -----------------------------------------------------------------
! Determine grid for input data 
!
! Data are 10 x 10 minute, stored in latitude bands,
! from south to north. In a given latitude band, data begin
! at dateline and proceed eastward.
! -----------------------------------------------------------------
...
! -----------------------------------------------------------------
  ...
  call wrap_put_att_text (ncid, nf_global, 'data_type', 'elevation_data')
...
! Define input file specific variables
  ...
  dim2_id(1) = lon_id
  dim2_id(2) = lat_id
  call wrap_def_var (ncid ,'ELEVATION' ,nf_float, 2, dim2_id, elev_id)
  call wrap_put_att_text (ncid, elev_id, 'long_name', name)
  call wrap_put_att_text (ncid, elev_id, 'units'    , unit)
...
! Read in formatted surface data
  ...
  do j = 1, nlat
     do i = 1, nlon
        read (ndata,*) templandmask(i,j), tempelev(i,j)
     end do
  end do
  close(ndata)
  do j = 1, nlat
     do i = 1, nlon
        if (i>=1 .and. i<=nlon/2) then
           landmask(i+1080,j) = templandmask(i,j)
           elev(i+1080,j) = tempelev(i,j)
        else
           landmask(i-1080,j) = templandmask(i,j)
           elev(i-1080,j) = tempelev(i,j)
        end if
...
! Create output file
  ...
  call wrap_put_var_realx (ncid, elev_id    , elev)
  call wrap_put_var_realx (ncid, landmask_id, landmask)

  call wrap_close(ncid)

end program convert_navytopo
