...
! File specific settings
  ...
  real(r8) :: pct_irrig(nlon,nlat)        !pct irrig
  real(r8) :: landmask(nlon,nlat)         !landmask
  ...
  integer :: pct_irrig_id                 !pct_irrig id
...
! Determine input and output file names
  ...
  filei = '/ptmp/slevis/lsmv2_2/input/0.5x0.5/olsonirrig.dat'
  fileo = '/ptmp/slevis/lsmv2_2/input/0.5x0.5/new/mksrf_irrig.nc'
...
! -----------------------------------------------------------------
  ...
  call wrap_put_att_text (ncid, nf_global, 'data_type', 'olson_irrig_data')
...
! Define percent irrig variables

  name = 'percent irrigated'
  unit = 'unitless'
  dim2_id(1) = lon_id
  dim2_id(2) = lat_id
  call wrap_def_var (ncid ,'PCT_IRRIG' ,nf_float, 2, dim2_id, pct_irrig_id)
  call wrap_put_att_text (ncid, pct_irrig_id, 'long_name', name)
  call wrap_put_att_text (ncid, pct_irrig_id, 'units'    , unit)
...
! Read in formatted surface data
  ...
  do j = 1, nlat
     do i = 1, nlon
        read (ndata,*) landmask(i,j),pct_irrig(i,j)
     end do
  end do
  close(ndata)
...
! Create output file
  ...
  call wrap_put_var_realx (ncid, pct_irrig_id  , pct_irrig)
  call wrap_put_var_realx (ncid, landmask_id   , landmask)

  call wrap_close(ncid)

end program convert_irrig
