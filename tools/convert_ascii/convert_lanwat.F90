...
! Determine input and output file names
  ...
  filei = '/ptmp/slevis/lsminput/temp/cogley.dat'
  fileo = '/ptmp/slevis/lsminput/temp/mksrf_lanwat.nc'
...
! -----------------------------------------------------------------
  ...
  call wrap_put_att_text (ncid, nf_global, 'data_type', 'cogley_lanwat_data')
...
! Read in formatted surface data
  ...
  do j = 1, nlat
     do i = 1, nlon
        read (ndata,*) landmask(i,j),pct_lake(i,j),pct_wetland(i,j)
!       if (landmask(i,j) < 50.) then !landmask in cogley.dat doesn't incl lake
        if ((landmask(i,j) + pct_lake(i,j)) < 50.) then
           landmask(i,j) = 0.
           pct_lake(i,j) = 0.
           pct_wetland(i,j) = 0.
        else
