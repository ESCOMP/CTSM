...
subroutine mkmap(filei, fileo, fmap, ftemplate)
  ...
  integer :: nlen
  integer :: nlon, nlat
  integer :: nlonatm,nlatatm
  integer :: dimid
  integer :: dimid_lndgrid, dimid_atmgrid
  integer :: dimid_lon, dimid_lat
  integer :: dimid_lonatm, dimid_latatm
  ...
  ! TODO: Need to put in check for input dataset that this is the case
  nlonatm = nlon
  nlatatm = nlat  
  ...
  do nd = 1,nDimensions
     ...
     ! Assume that input dimensions are time, lndgrid, atmgrid
     ! 2d lon,lat       <=> 1d lndgrid
     ! 2d lonatm,latatm <=> 1d atmgrid
     if (dimname == 'time') then
        ...
        else if (trim(dimname) == 'atmgrid') then
           dimid_atmgrid= nd
           call handle_ncerr(nf90_def_dim(ncido, name='lonatm', len=nlon, dimid=dimid_lonatm)) 
           call handle_ncerr(nf90_def_dim(ncido, name='latatm', len=nlat, dimid=dimid_latatm)) 
        else
  ...
  do nv = 1,nVariables
     ...
     if (ignore_var(varname)) then
        ...
        else if (dimidsi(1) == dimid_atmgrid) then
           ndimso = ndimsi + 1
           dimidso(1) = dimid_lonatm
           dimidso(2) = dimid_latatm
           do n = 2,ndimsi
              call handle_ncerr(nf90_inquire_dimension(ncidi, dimidsi(n), dimname))
              call handle_ncerr(nf90_inq_dimid(ncido, dimname, dimidso(n+1))) 
           end do
...
contains
   ...
   logical function ignore_var(varname)
      ...
      ! We exclude areaatm, areaupsc and indxupsc, because these area-related variables
      ! are confusing (and in the case of indxupsc, it can't be regridded properly). But
      ...
      ignore_var = (trim(varname) == 'date_written' .or. &
                    ...
                    trim(varname) == 'lon' .or. &
                    trim(varname) == 'lat' .or. &
                    trim(varname) == 'lonatm' .or. &
                    trim(varname) == 'latatm' .or. &
                    trim(varname) == 'longxy' .or. &
                    trim(varname) == 'latixy' .or. &
                    trim(varname) == 'longxyatm' .or. &
                    trim(varname) == 'latixyatm' .or. &
                    trim(varname) == 'areaatm' .or. &
                    trim(varname) == 'areaupsc' .or. &
                    trim(varname) == 'indxupsc' .or. &
  ...
  subroutine outputvar(varidi, varido, nlon, nlat, ncidi, ncido, tgridmap)
    ...
    if (trim(dimname)=='lndgrid' .or. trim(dimname)=='atmgrid')  then 
       mapvar = .true.
    else
       mapvar = .false.
    end if
