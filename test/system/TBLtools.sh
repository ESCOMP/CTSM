...
if [ -n "${BL_ROOT}" ]; then
    ...
    env CLM_TESTDIR=${BL_TESTDIR} \
        ...
        ${BL_ROOT}/test/system/TSMtools.sh $1 $2
    rc=$?
    if [ $rc -ne 0 ]; then
...
files_to_compare=`cd ${CLM_TESTDIR}/TSMtools.$1.$2; ls *.nc`
...
for compare_file in ${files_to_compare}; do
    ...
    ${CLM_SCRIPTDIR}/CLM_compare.sh \
        ${BL_TESTDIR}/TSMtools.$1.$2/${compare_file} \
        ${CLM_TESTDIR}/TSMtools.$1.$2/${compare_file}
    rc=$?
