...
!        npts_dest = npts_dest)
    ...
    ! TODO After committing this code that works, comment out these lines,
    !      uncomm the lines above, add the dzsoi args on both sides of the call
    allocate(dzsoi_source_all, source=coordinates_source_all)
    allocate(dzsoi_dest_all, source=coordinates_dest_all)
    dzsoi_source_all(:,:) = 1._r8
    dzsoi_dest_all(:,:) = 1._r8
