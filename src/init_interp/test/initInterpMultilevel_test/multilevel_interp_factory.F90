...
contains
  ...
  function create_multilevel_interp_no_levclasses(coordinates_source, coordinates_dest, &
       index_source, index_dest, npts_source, npts_dest) &
       result(interpolator)
    ! Arguments:
    type(interp_multilevel_interp_type) :: interpolator  ! function result
    real(r8), intent(in) :: coordinates_source(:)  ! coordinates in source data for index_source
    real(r8), intent(in) :: coordinates_dest(:)    ! coordinates in dest data for index_dest

    integer, intent(in) :: index_source  ! source index of interest in tests
    integer, intent(in) :: index_dest    ! dest index of interest in tests
    integer, intent(in) :: npts_source   ! total number of points wanted in source
  ...
  function create_multilevel_interp_with_levclasses(coordinates_source, coordinates_dest, &
    ...
    real(r8), intent(in) :: coordinates_source(:)   ! coordinates in source data for index_source
    real(r8), intent(in) :: coordinates_dest(:)     ! coordinates in dest data for index_dest
    integer , intent(in) :: level_classes_source(:) ! class indices in source data for index_source
  ...
  subroutine create_coordinate_arrays(coordinates_source, coordinates_dest, &
    ...
    ! But put the passed-in coordinates in index_source & index_dest:
    coordinates_source_all(:, index_source) = coordinates_source(:)
  ...
  subroutine create_class_arrays(level_classes_source, level_classes_dest, &
       ...
       index_source, index_dest, npts_source, npts_dest)
    ! Arguments:
    integer, intent(in) :: level_classes_source(:)  ! classes in source data for index_source
    ...
    allocate(level_classes_source_all(nlevels_source, npts_source), source = default_class)
    allocate(level_classes_dest_all(nlevels_dest, npts_dest), source = default_class)

    level_classes_source_all(:, index_source) = level_classes_source(:)
