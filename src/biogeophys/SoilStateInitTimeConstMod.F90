...
! remove roots below bedrock layer
    ...
    do c = begc,endc
       ...
       !soilstate_inst%gwc_thr_col(c) = 0.01_r8*(0.17_r8*clay3d(g,1) + 0.0014_r8*clay3d(g,1)*clay3d(g,1))  ! Jasper Kok et al. (2014) modified the equation of soil moisture effect for dust emissions using a tuning factor of a = 1. 0.01 is to convert the threshold gravimetric soil moisture from % to fraction. Danny M. Leung et al. (2023) followed K14 and used this equation for the paper's results. However, Danny Leung later (Dec 2023) decided to revert to Zender's choice and use a = 1 / (clay fraction), which overall encourages more dust emissions from seriamid and more marginal dust sources. dmleung added this detailed comment on 19 Feb 2024.
