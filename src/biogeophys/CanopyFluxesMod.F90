...
!!$            if(abs(cp_stem(p)) > 0. ) then
!!$            else
!!$               write(iulog,*) 'badcp: ', c, p, cp_stem(p)
!!$               write(iulog,*) 'badcp2: ', stem_biomass(p),fbw(patch%itype(p))
!!$            endif
...
! should be the same expression used in Photosynthesis/getqflx
            ...
            if(t_veg(p) > 0. .and. t_veg(p) < 1000.) then
            else
               write(iulog,*) 'badtemp: ', c, p, t_veg(p),fstem(p)
               write(iulog,'(a12,i4,6f12.6)') 'badtemp2: ', &
                    patch%itype(p),- 4._r8*bir(p), &
                    4._r8*sa_internal(p)*emv(p)*sb,  &
                    dc1*wtga(p) ,dc2*wtgaq*qsatldT(p) , cp_veg(p)/dtime
!               write(iulog,'(a12,i4,6f12.6)') 'badtemp3: ',patch%itype(p),wtga(p),wta0(p),wtg0,wtstem0(p),rstema,rb(p)

                write(iulog,'(a12,i4,6f12.6)') 'badtemp3: ', &
                    patch%itype(p),dbh(p),dbh_param(patch%itype(p))
            endif
      ...
      do f = 1, fn
         ...
         if(t_stem(p) > 0. .and. t_stem(p) < 1000.) then
            else
               write(iulog,'(a12,3i8,6f12.6)') 'badtemp4: ', c, p, patch%itype(p), t_stem(p), cp_stem(p)
               write(iulog,'(a12,i8,6f12.6)') 'badtemp5: ', patch%itype(p), fstem(p),stem_biomass(p), cp_veg(p),leaf_biomass(p)
            endif
