...
!!
  ...
  subroutine hybrid(x0, p, iv, c, gb_mol, je, cair, oair, lmr_z, par_z,&
    ...
    real(r8), intent(inout) :: gs_mol          ! leaf stomatal conductance (umol H2O/m**2/s) (inout instead of out to prevent returning nan)
...
! explicitly set conductance to zero for top soil layer
  ...
  subroutine brent_PHS(xsun, x1sun, x2sun, f1sun, f2sun, xsha, x1sha, x2sha, f1sha, f2sha, &
    ...
    real(r8), intent(in)    :: rh_can               ! inside canopy relative humidity
    real(r8), intent(inout) :: gs_mol_sun           ! sunlit leaf stomatal conductance (umol H2O/m**2/s) (inout instead of out to prevent returning nan)
    real(r8), intent(inout) :: gs_mol_sha           ! shaded leaf stomatal conductance (umol H2O/m**2/s) (inout instead of out to prevent returning nan)
