...
contains
  ...
  subroutine CNFireArea (bounds, num_soilc, filter_soilc, num_soilp, filter_soilp)
   ...
   lfpftd           =>    pps%lfpftd               , & ! Input:  [real(r8) (:)]  decrease of pft weight (0-1) on the col. for dt
   ...
   farea_burned     =>    cps%farea_burned         , & ! Output:  [real(r8) (:)]  total fractional area burned (/sec)
   baf_crop         =>    cps%baf_crop             , & ! Output:  [real(r8) (:)]  burned area fraction for cropland (/sec)
   baf_peatf        =>    cps%baf_peatf            , & ! Output:  [real(r8) (:)]  burned area fraction for peatland (/sec)
   fbac             =>    cps%fbac                 , & ! Output:  [real(r8) (:)]  total burned area out of conversion (/sec)
   fbac1            =>    cps%fbac1             , & ! Output: [real(r8) (:)] burned area out of conv. region due to LU fire (/sec)
   cropf_col        =>    cps%cropf_col            , & ! Output:  [real(r8) (:)]  cropland fraction in veg column
   gdp_lf           =>    cps%gdp_lf               , & ! Input:  [real(r8) (:)]  gdp data
   peatf_lf         =>    cps%peatf_lf             , & ! Input:  [real(r8) (:)]  peatland fraction data
   abm_lf           =>    cps%abm_lf               , & ! Input:  [integer (:)]  prescribed crop fire time
   nfire            =>    cps%nfire                , & ! Output:  [real(r8) (:)]  fire counts (count/km2/sec), valid only in Reg. C
   totlitc          =>    ccs%totlitc              , & ! Input:  [real(r8) (:)]  (gC/m2) total lit C (column-level mean)
   fsr_col          =>    cps%fsr_col              , & ! Output:  [real(r8) (:)]  fire spread rate at column level
   fd_col           =>    cps%fd_col               , & ! Output:  [real(r8) (:)]  fire duration at column level
   rootc_col        =>    ccs%rootc_col            , & ! Output:  [real(r8) (:)]  root carbon
   totvegc_col      =>    ccs%totvegc_col          , & ! Output:  [real(r8) (:)]  totvegc at column level
   leafc_col        =>    ccs%leafc_col            , & ! Output:  [real(r8) (:)]  leaf carbon at column level
   lgdp_col         =>    cps%lgdp_col             , & ! Output:  [real(r8) (:)]  gdp limitation factor for nfire
   lgdp1_col        =>    cps%lgdp1_col            , & ! Output:  [real(r8) (:)]  gdp limitation factor for baf per fire
   lpop_col         =>    cps%lpop_col             , & ! Output:  [real(r8) (:)]  pop limitation factor for baf per fire
   fuelc            =>    ccs%fuelc                , & ! Output:  [real(r8) (:)]  fuel avalability factor for Reg.C
   fuelc_crop       =>    ccs%fuelc_crop           , & ! Output:  [real(r8) (:)]  fuel avalability factor for Reg.A
   btran_col        =>    cps%btran_col            , & ! Output:  [real(r8) (:)]  transpiration wetness factor (0 to 1)
   wtlf             =>    cps%wtlf                 , & ! Output:  [real(r8) (:)]  fractional coverage of non-crop PFTs
   lfwt             =>    cps%lfwt                 , & ! Output:  [real(r8) (:)]  frac. coverage of non-crop and non-bare-soil PFTs
   trotr1_col       =>    cps%trotr1_col           , & ! Output:  [real(r8) (:)]  pft weight of BET on the gridcell (0-1)
   trotr2_col       =>    cps%trotr2_col           , & ! Output:  [real(r8) (:)]  pft weight of BDT on the gridcell (0-1)
   dtrotr_col       =>    cps%dtrotr_col           , & ! Output:  [real(r8) (:)]  decreased frac. coverage of BET+BDT on grid for dt
   prec60_col       =>    cps%prec60_col           , & ! Output:  [real(r8) (:)]  60-day running mean of tot. precipitation
   prec10_col       =>    cps%prec10_col           , & ! Output:  [real(r8) (:)]  10-day running mean of tot. precipitation
   lfc              =>    cps%lfc              , & ! InOut: [real(r8) (:)] conv. area frac. of BET+BDT that haven't burned before
 ...
 subroutine CNFireFluxes (num_soilc, filter_soilc, num_soilp, filter_soilp)
   ...
   farea_burned     =>    cps%farea_burned                  , & ! Input:  [real(r8) (:)]  fractional area burned (/sec)
   fire_mortality_c_to_cwdc => ccf%fire_mortality_c_to_cwdc , & ! Output: [real(r8) (:,:)] C flux fire mortality to CWD (gC/m3/s)
   fire_mortality_n_to_cwdn => cnf%fire_mortality_n_to_cwdn , & ! Output: [real(r8) (:,:)] N flux fire mortality to CWD (gN/m3/s)
   lfc              =>    cps%lfc              , & ! InOut:  [real(r8) (:)]  conv. area frac. of BET+BDT that haven't burned before
   lfc2             =>    cps%lfc2             , & ! Output:  [real(r8) (:)]  conv. area frac. of BET+BDT burned this dt (/sec)
   fbac1            =>    cps%fbac1                  , & ! Input:  [real(r8) (:)]  burned area out of conv. region due to LU fire
   fbac             =>    cps%fbac                   , & ! Input:  [real(r8) (:)]  total burned area out of conversion
   baf_crop         =>    cps%baf_crop               , & ! Input:  [real(r8) (:)]  BAF for cropland
   baf_peatf        =>    cps%baf_peatf              , & ! Input:  [real(r8) (:)]  BAF for peatland
   leafcmax         =>    pcs%leafcmax               , & ! InOut:  [real(r8) (:)]  (gC/m2) ann max leaf C
   ...
   dtrotr_col       =>    cps%dtrotr_col         , & ! Input:  [real(r8) (:)]  ann. decreased frac. coverage of BET+BDT (0-1) on GC
   somc_fire        =>    ccf%somc_fire              , & ! Output:  [real(r8) (:)]  (gC/m2/s)fire C emissions due to peat burning
   totsomc          =>    ccs%totsomc                , & ! Input:  [real(r8) (:)]  (gC/m2) total soil organic matter C
   decomp_cpools_vr =>    ccs%decomp_cpools_vr       , & ! Input:  [real(r8) (:,:,:)]  (gC/m3)  VR decomp. (litter, cwd, soil)
   decomp_npools_vr =>    cns%decomp_npools_vr       , & ! Input:  [real(r8) (:,:,:)]  (gC/m3)  VR decomp. (litter, cwd, soil)
   leafc            =>    pcs%leafc                  , & ! Input:  [real(r8) (:)]  (gC/m2) leaf C
   leafc_storage    =>    pcs%leafc_storage          , & ! Input:  [real(r8) (:)]  (gC/m2) leaf C storage
   leafc_xfer       =>    pcs%leafc_xfer             , & ! Input:  [real(r8) (:)]  (gC/m2) leaf C transfer
   ...
   m_leafc_to_fire    =>    pcf%m_leafc_to_fire             , & ! Output:  [real(r8) (:)]  (gC/m2/s) fire C emis. from leafc
   m_leafc_storage_to_fire=>pcf%m_leafc_storage_to_fire , & ! Output:  [real(r8) (:)]  (gC/m2/s) fire C emis. from leafc_storage
   m_leafc_xfer_to_fire=>    pcf%m_leafc_xfer_to_fire       , & ! Output:  [real(r8) (:)]  (gC/m2/s) fire C emis. from leafc_xfer
   m_livestemc_to_fire=>    pcf%m_livestemc_to_fire         , & ! Output:  [real(r8) (:)]  (gC/m2/s) fire C emis. from livestemc
   ! Fire emissions from given type
   m_livestemc_storage_to_fire=> pcf%m_livestemc_storage_to_fire , & ! Output:  [real(r8) (:)]  (gC/m2/s) C emis. livestemc_storage
   m_livestemc_xfer_to_fire=>    pcf%m_livestemc_xfer_to_fire       , & ! Output:  [real(r8) (:)]  (gC/m2/s) C emis. livestemc_xfer
   m_deadstemc_to_fire=>    pcf%m_deadstemc_to_fire                 , & ! Output:  [real(r8) (:)]  (gC/m2/s) C emis. deadstemc_xfer
   m_deadstemc_storage_to_fire=> pcf%m_deadstemc_storage_to_fire , & ! Output:  [real(r8) (:)]  (gC/m2/s) C emis. deadstemc_storage
   m_deadstemc_xfer_to_fire=>    pcf%m_deadstemc_xfer_to_fire    , & ! Output:  [real(r8) (:)]  (gC/m2/s) C emis. deadstemc_xfer
   m_frootc_to_fire=>    pcf%m_frootc_to_fire                        , & ! Output:  [real(r8) (:)]  (gC/m2/s) C emis. frootc
   m_frootc_storage_to_fire=>    pcf%m_frootc_storage_to_fire    , & ! Output:  [real(r8) (:)]  (gC/m2/s) C emis. frootc_storage
   m_frootc_xfer_to_fire=>    pcf%m_frootc_xfer_to_fire              , & ! Output:  [real(r8) (:)]  (gC/m2/s) C emis. frootc_xfer
   m_livecrootc_to_fire =>    pcf%m_livecrootc_to_fire               , & ! Output:  [real(r8) (:)]  (gC/m2/s) C emis. livecrootc
   m_livecrootc_storage_to_fire=>pcf%m_livecrootc_storage_to_fire, & ! Output: [real(r8) (:)]  (gC/m2/s) C emis. livecrootc_storage
   m_livecrootc_xfer_to_fire   =>pcf%m_livecrootc_xfer_to_fire   , & ! Output: [real(r8) (:)]  (gC/m2/s) C emis. livecrootc_xfer
   m_deadcrootc_to_fire        =>    pcf%m_deadcrootc_to_fire        , & ! Output:  [real(r8) (:)]  (gC/m2/s) C emis. deadcrootc
   m_deadcrootc_storage_to_fire=>pcf%m_deadcrootc_storage_to_fire, & ! Output: [real(r8) (:)]  (gC/m2/s) C emis. deadcrootc_storage
   m_deadcrootc_xfer_to_fire   =>    pcf%m_deadcrootc_xfer_to_fire   , & ! Output: [real(r8) (:)] (gC/m2/s) C emis. deadcrootc_xfer
   m_gresp_storage_to_fire     =>    pcf%m_gresp_storage_to_fire     , & ! Output:  [real(r8) (:)]  (gC/m2/s) C emis. gresp_storage
   m_gresp_xfer_to_fire        =>    pcf%m_gresp_xfer_to_fire        , & ! Output:  [real(r8) (:)]  (gC/m2/s) C emis. gresp_xfer

   ! Nitrogen fire emissions from given type
   m_leafn_to_fire             => pnf%m_leafn_to_fire             , & ! Output:  [real(r8) (:)]  (gN/m2/s) N emis. leafn
   m_leafn_storage_to_fire     => pnf%m_leafn_storage_to_fire     , & ! Output:  [real(r8) (:)]  (gN/m2/s) N emis. leafn_storage
   m_leafn_xfer_to_fire        => pnf%m_leafn_xfer_to_fire        , & ! Output:  [real(r8) (:)]  (gN/m2/s) N emis. leafn_xfer
   m_livestemn_to_fire         => pnf%m_livestemn_to_fire         , & ! Output:  [real(r8) (:)]  (gN/m2/s) N emis. livestemn
   m_livestemn_storage_to_fire => pnf%m_livestemn_storage_to_fire , & ! Output: [real(r8) (:)] (gN/m2/s) N emis. livestemn_s
   m_livestemn_xfer_to_fire    => pnf%m_livestemn_xfer_to_fire    , & ! Output:  [real(r8) (:)]  (gN/m2/s) N emis. livestemn_xfer
   m_deadstemn_to_fire         => pnf%m_deadstemn_to_fire         , & ! Output:  [real(r8) (:)]  (gN/m2/s) N emis. deadstemn
   m_deadstemn_storage_to_fire => pnf%m_deadstemn_storage_to_fire , & ! Output:  [real(r8) (:)]  (gN/m2/s) N emis. deadstemn_storage
   m_deadstemn_xfer_to_fire    => pnf%m_deadstemn_xfer_to_fire    , & ! Output:  [real(r8) (:)]  (gN/m2/s) N emis. deadstemn_xfer
   m_frootn_to_fire            => pnf%m_frootn_to_fire            , & ! Output:  [real(r8) (:)]  (gN/m2/s) N emis. frootn
   m_frootn_storage_to_fire    => pnf%m_frootn_storage_to_fire    , & ! Output:  [real(r8) (:)]  (gN/m2/s) N emis. frootn_storage
   m_frootn_xfer_to_fire       => pnf%m_frootn_xfer_to_fire       , & ! Output:  [real(r8) (:)]  (gN/m2/s) N emis. frootn_xfer
   m_livecrootn_to_fire        => pnf%m_livecrootn_to_fire  , & ! Output:  [real(r8) (:)]  (gN/m2/s) N emis. m_livecrootn_to_fire
   m_livecrootn_storage_to_fire=> pnf%m_livecrootn_storage_to_fire, & ! Output: [real(r8) (:)] (gN/m2/s) N emis. livecrootn_storage
   m_livecrootn_xfer_to_fire   => pnf%m_livecrootn_xfer_to_fire   , & ! Output:  [real(r8) (:)]  (gN/m2/s) N emis. livecrootn_xfer
   m_deadcrootn_to_fire        => pnf%m_deadcrootn_to_fire        , & ! Output:  [real(r8) (:)]  (gN/m2/s) N emis. deadcrootn
   m_deadcrootn_storage_to_fire=> pnf%m_deadcrootn_storage_to_fire, & ! Output: [real(r8) (:)] (gN/m2/s) N emis. deadcrootn_storage
   m_deadcrootn_xfer_to_fire   => pnf%m_deadcrootn_xfer_to_fire   , & ! Output:  [real(r8) (:)]  (gN/m2/s) N emis. deadcrootn_xfer
   m_retransn_to_fire          => pnf%m_retransn_to_fire          , & ! Output:  [real(r8) (:)]  (gN/m2/s) N emis. retransn

   m_leafc_to_litter_fire      =>    pcf%m_leafc_to_litter_fire                  , & ! Output:  [real(r8) (:)]
   m_leafc_storage_to_litter_fire=>    pcf%m_leafc_storage_to_litter_fire        , & ! Output:  [real(r8) (:)]
   m_leafc_xfer_to_litter_fire =>    pcf%m_leafc_xfer_to_litter_fire             , & ! Output:  [real(r8) (:)]
   m_livestemc_to_litter_fire  =>    pcf%m_livestemc_to_litter_fire              , & ! Output:  [real(r8) (:)]
   m_livestemc_storage_to_litter_fire=>    pcf%m_livestemc_storage_to_litter_fire, & ! Output:  [real(r8) (:)]
   m_livestemc_xfer_to_litter_fire=>    pcf%m_livestemc_xfer_to_litter_fire      , & ! Output:  [real(r8) (:)]
   m_livestemc_to_deadstemc_fire=>    pcf%m_livestemc_to_deadstemc_fire          , & ! Output:  [real(r8) (:)]
   m_deadstemc_to_litter_fire=>    pcf%m_deadstemc_to_litter_fire                , & ! Output:  [real(r8) (:)]
   m_deadstemc_storage_to_litter_fire=>    pcf%m_deadstemc_storage_to_litter_fire, & ! Output:  [real(r8) (:)]
   m_deadstemc_xfer_to_litter_fire=>    pcf%m_deadstemc_xfer_to_litter_fire      , & ! Output:  [real(r8) (:)]
   m_frootc_to_litter_fire   =>    pcf%m_frootc_to_litter_fire                   , & ! Output:  [real(r8) (:)]
   m_frootc_storage_to_litter_fire=>    pcf%m_frootc_storage_to_litter_fire      , & ! Output:  [real(r8) (:)]
   m_frootc_xfer_to_litter_fire=>    pcf%m_frootc_xfer_to_litter_fire            , & ! Output:  [real(r8) (:)]
   m_livecrootc_to_litter_fire =>    pcf%m_livecrootc_to_litter_fire             , & ! Output:  [real(r8) (:)]
   m_livecrootc_storage_to_litter_fire =>    pcf%m_livecrootc_storage_to_litter_fire, & ! Output:  [real(r8) (:)]
   m_livecrootc_xfer_to_litter_fire=>    pcf%m_livecrootc_xfer_to_litter_fire       , & ! Output:  [real(r8) (:)]
   m_livecrootc_to_deadcrootc_fire=>    pcf%m_livecrootc_to_deadcrootc_fire         , & ! Output:  [real(r8) (:)]
   m_deadcrootc_to_litter_fire=>    pcf%m_deadcrootc_to_litter_fire              , & ! Output:  [real(r8) (:)]
   m_deadcrootc_storage_to_litter_fire =>    pcf%m_deadcrootc_storage_to_litter_fire, & ! Output:  [real(r8) (:)]
   m_deadcrootc_xfer_to_litter_fire=>    pcf%m_deadcrootc_xfer_to_litter_fire    , & ! Output:  [real(r8) (:)]
   m_gresp_storage_to_litter_fire=>    pcf%m_gresp_storage_to_litter_fire        , & ! Output:  [real(r8) (:)]
   m_gresp_xfer_to_litter_fire   =>    pcf%m_gresp_xfer_to_litter_fire           , & ! Output:  [real(r8) (:)]
   m_decomp_cpools_to_fire_vr => ccf%m_decomp_cpools_to_fire_vr , & ! Output:  [real(r8) (:,:,:)]  (gC/m3/s) VR decomp. C fire loss
   m_c_to_litr_met_fire       => ccf%m_c_to_litr_met_fire       , & ! Output:  [real(r8) (:,:)]
   m_c_to_litr_cel_fire       => ccf%m_c_to_litr_cel_fire       , & ! Output:  [real(r8) (:,:)]
   m_c_to_litr_lig_fire       => ccf%m_c_to_litr_lig_fire       , & ! Output:  [real(r8) (:,:)]

   m_leafn_to_litter_fire     => pnf%m_leafn_to_litter_fire                   , & ! Output:  [real(r8) (:)]
   m_leafn_storage_to_litter_fire=>    pnf%m_leafn_storage_to_litter_fire          , & ! Output:  [real(r8) (:)]
   m_leafn_xfer_to_litter_fire  =>    pnf%m_leafn_xfer_to_litter_fire              , & ! Output:  [real(r8) (:)]
   m_livestemn_to_litter_fire   =>    pnf%m_livestemn_to_litter_fire               , & ! Output:  [real(r8) (:)]
   m_livestemn_storage_to_litter_fire=>    pnf%m_livestemn_storage_to_litter_fire   , & ! Output:  [real(r8) (:)]
   m_livestemn_xfer_to_litter_fire=>    pnf%m_livestemn_xfer_to_litter_fire         , & ! Output:  [real(r8) (:)]
   m_livestemn_to_deadstemn_fire  =>    pnf%m_livestemn_to_deadstemn_fire           , & ! Output:  [real(r8) (:)]
   m_deadstemn_to_litter_fire     =>    pnf%m_deadstemn_to_litter_fire              , & ! Output:  [real(r8) (:)]
   m_deadstemn_storage_to_litter_fire=>    pnf%m_deadstemn_storage_to_litter_fire   , & ! Output:  [real(r8) (:)]
   m_deadstemn_xfer_to_litter_fire=>    pnf%m_deadstemn_xfer_to_litter_fire         , & ! Output:  [real(r8) (:)]
   m_frootn_to_litter_fire        =>    pnf%m_frootn_to_litter_fire                 , & ! Output:  [real(r8) (:)]
   m_frootn_storage_to_litter_fire=>    pnf%m_frootn_storage_to_litter_fire         , & ! Output:  [real(r8) (:)]
   m_frootn_xfer_to_litter_fire   =>    pnf%m_frootn_xfer_to_litter_fire            , & ! Output:  [real(r8) (:)]
   m_livecrootn_to_litter_fire    =>    pnf%m_livecrootn_to_litter_fire             , & ! Output:  [real(r8) (:)]
   m_livecrootn_storage_to_litter_fire =>    pnf%m_livecrootn_storage_to_litter_fire , & ! Output:  [real(r8) (:)]
   m_livecrootn_xfer_to_litter_fire    =>    pnf%m_livecrootn_xfer_to_litter_fire    , & ! Output:  [real(r8) (:)]
   m_livecrootn_to_deadcrootn_fire     =>    pnf%m_livecrootn_to_deadcrootn_fire     , & ! Output:  [real(r8) (:)]
   m_deadcrootn_to_litter_fire         =>    pnf%m_deadcrootn_to_litter_fire         , & ! Output:  [real(r8) (:)]
   m_deadcrootn_storage_to_litter_fire =>    pnf%m_deadcrootn_storage_to_litter_fire , & ! Output:  [real(r8) (:)]
   m_deadcrootn_xfer_to_litter_fire=>    pnf%m_deadcrootn_xfer_to_litter_fire        , & ! Output:  [real(r8) (:)]
   m_retransn_to_litter_fire       =>    pnf%m_retransn_to_litter_fire  , & ! Output:  [real(r8) (:)]
   m_decomp_npools_to_fire_vr =>cnf%m_decomp_npools_to_fire_vr , & ! Output:  [real(r8) (:,:,:)]  VR decomp. N fire loss (gN/m3/s)
   m_n_to_litr_met_fire       =>cnf%m_n_to_litr_met_fire       , & ! Output:  [real(r8) (:,:)]
   m_n_to_litr_cel_fire       =>cnf%m_n_to_litr_cel_fire       , & ! Output:  [real(r8) (:,:)]
   m_n_to_litr_lig_fire       =>cnf%m_n_to_litr_lig_fire       , & ! Output:  [real(r8) (:,:)]
