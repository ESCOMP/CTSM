...
! ============================================================================
  ...
  real(r8) function Hite( cohort_in )
    ...
    if(cohort_in%dbh <= EDpftcon%max_dbh(cohort_in%pft)) then
       h = (10.0_r8**(log10(cohort_in%dbh) * m + c))
    else 
       h = (10.0_r8**(log10(EDpftcon%max_dbh(cohort_in%pft))*m + c))
    endif
    Hite = h 
...
! ============================================================================
  ...
  real(r8) function Bleaf( cohort_in )
    ...
    if(cohort_in%dbh <= EDpftcon%max_dbh(cohort_in%pft))then
       bleaf = 0.0419_r8 * (cohort_in%dbh**1.56) * EDpftcon%wood_density(cohort_in%pft)**0.55_r8
    else  
       bleaf = 0.0419_r8 * (EDpftcon%max_dbh(cohort_in%pft)**1.56) * EDpftcon%wood_density(cohort_in%pft)**0.55_r8       
    endif
...
! ============================================================================
  ...
  real(r8) function c_area( cohort_in )
    ...
    if (DEBUG_growth) then
       ...
       write(iulog,*) 'z_area 2',EDpftcon%max_dbh
    ...
    dbh = min(cohort_in%dbh,EDpftcon%max_dbh(cohort_in%pft))
...
! ============================================================================
  ...
  real(r8) function Bdead( cohort_in )
    ...
    bdead = 0.06896_r8*(cohort_in%hite**0.572_r8)*(cohort_in%dbh**1.94_r8)* &
         (EDpftcon%wood_density(cohort_in%pft)**0.931_r8)
  ...
  real(r8) function dDbhdBd( cohort_in )
    ...
    if(cohort_in%dbh < EDpftcon%max_dbh(cohort_in%pft))then
       ...
       dBD_dDBH = dBD_dDBH + 0.572_r8*0.06896_r8*(cohort_in%hite**(0.572_r8 - 1.0_r8))* &
            (cohort_in%dbh**1.94_r8)*(EDpftcon%wood_density(cohort_in%pft)**0.931_r8)*dH_dDBH
    endif
...
! ============================================================================
  ...
  real(r8) function dDbhdBl( cohort_in )
    ...
    dblddbh = 1.56_r8*0.0419_r8*(cohort_in%dbh**0.56_r8)*(EDpftcon%wood_density(cohort_in%pft)**0.55_r8)
...
! ============================================================================
  ...
  real(r8) function mortality_rates( cohort_in )
    ...
    bmort = 0.014_r8 !+ 0.15_r8*(1.0_r8-EDpftcon%wood_density(cohort_in%pft))
