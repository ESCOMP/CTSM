...
contains
  ...
  !          Returns time differentials of the state vector
  ! ============================================================================
  subroutine non_canopy_derivs( currentPatch )
             ...
             if(currentCohort%year_net_uptake(z) /= 999._r8)then !there was activity this year in this leaf layer. 
                ...
                if(pftcon%season_decid(currentCohort%pft) == 1.or.pftcon%stress_decid(currentCohort%pft) == 1)then 
                   ...
                   currentCohort%leaf_cost = currentCohort%leaf_cost + 1.0_r8/(pftcon%slatop(currentCohort%pft)*1000_r8) * &
                        pftcon%froot_leaf(currentCohort%pft) / EDpftcon%root_long(currentCohort%pft)
                ...
                if(currentCohort%year_net_uptake(z) < currentCohort%leaf_cost)then
                   if(currentCohort%canopy_trim > trim_limit)then
                      ...
                      if(currentCohort%hite > EDpftcon%hgt_min(currentCohort%pft))then
  ...
  !                     Phenology. 
  ! ============================================================================
  subroutine phenology( g )

    use clmtype,     only : pvs, pps, pftcon, cws
    ...
    if(GDD0(currentSite%oldest_patch%clm_pno) > gdd_threshold.and.currentSite%status == 1)then
       if(timesinceleafoff > 0.9*(365-365/(1.0_r8/pftcon%leaf_long(7))))then  
    ...
    if(timesinceleafon > 365/(1.0_r8/pftcon%leaf_long(7)))then !!fudge - this  shouldn't be hard  wired. 
       if(currentSite%status == 2)then
          currentSite%status = 1        !alter status of site to 'leaves on'
          currentSite%leafoffdate = t   !record leaf off date   
          currentSite%gdd = 0._r8           !zero the GDD counter
    ...
    currentSite%water_memory(1) = cws%h2osoi_vol(currentSite%clmcolumn,1) 
  ...
  subroutine seeds_in( cp_pnt )
    ...
    do while(associated(currentPatch))
       if(EXTERNAL_RECRUITMENT == 1) then !external seed rain - needed to prevent extinction  
          do p = 1,numpft_ed
           currentPatch%seeds_in(p) = currentPatch%seeds_in(p) + EDpftcon%seed_rain(p) !KgC/m2/year
          enddo
       endif
       currentPatch => currentPatch%younger
    enddo
  ...
  subroutine Growth_Derivatives( currentCohort )
   ...
   ! calculate target size of living biomass compartment for a given dbh.   
    target_balive = Bleaf(currentCohort) * (1.0_r8 + pftcon%froot_leaf(currentCohort%pft) + &
         EDpftcon%sapwood_ratio(currentCohort%pft)*h)
    ...
    if(pftcon%evergreen(currentCohort%pft) == 1)then !grass and EBT
       ...
       currentCohort%root_md = currentCohort%br / EDpftcon%root_long(currentCohort%pft)
    ...
    currentCohort%carbon_balance = currentCohort%npp - currentCohort%md *  EDpftcon%leaf_stor_priority(currentCohort%pft)
    ...
    if(Bleaf(currentCohort) > 0._r8)then
       ...
       if(currentCohort%carbon_balance > 0._r8)then !spend C on growing and storing
          ...
          frac = max(0.0_r8,currentCohort%bstore/(Bleaf(currentCohort) * EDpftcon%cushion(currentCohort%pft)))
    ...
    balive_loss = 0._r8
    if(currentCohort%carbon_balance > currentCohort%md*(1.0_r8- EDpftcon%leaf_stor_priority(currentCohort%pft)))then ! Yes...
       currentCohort%carbon_balance = currentCohort%carbon_balance - currentCohort%md * (1.0_r8 - &
             EDpftcon%leaf_stor_priority(currentCohort%pft))
    else ! we can't maintain constant leaf area and root area. Balive is reduced
       balive_loss = currentCohort%md *(1.0_r8- EDpftcon%leaf_stor_priority(currentCohort%pft))- currentCohort%carbon_balance
       currentCohort%carbon_balance = 0._r8
    endif
    ...
    if((currentCohort%balive >= target_balive).AND.(currentCohort%carbon_balance >  0._r8))then 
       ...
       if(currentCohort%dbh <= EDpftcon%max_dbh(currentCohort%pft))then ! cap on leaf biomass
          ...
          dbswdbd = EDpftcon%sapwood_ratio(currentCohort%pft) * (h*dbldbd + currentCohort%bl*dhdbd_fn)
          u  = 1.0_r8 / (dbldbd + dbrdbd + dbswdbd)     
          va = 1.0_r8 / (1.0_r8 + u)
          vs = u / (1.0_r8 + u)
          gr_fract = 1.0_r8 - EDpftcon%seed_alloc(currentCohort%pft)
       else
          dbldbd = 0._r8; dbrdbd = 0._r8 ;dbswdbd = 0._r8      
          va = 0.0_r8
          vs = 1.0_r8
          gr_fract = 1.0_r8 - (EDpftcon%seed_alloc(currentCohort%pft) + EDpftcon%clone_alloc(currentCohort%pft))
       endif
  ...
  subroutine recruitment( t, currentPatch )
    ...
    do ft = 1,numpft_ed
       ...
       dc%pft         = ft
       dc%hite        = EDpftcon%hgt_min(ft)
       dc%dbh         = Dbh(dc)
       dc%bdead       = Bdead(dc)
       dc%balive      = Bleaf(dc)*(1.0_r8 + pftcon%froot_leaf(ft) + EDpftcon%sapwood_ratio(ft)*dc%hite)
       dc%bstore      = EDpftcon%cushion(ft)*(dc%balive/ (1.0_r8 + pftcon%froot_leaf(ft) + EDpftcon%sapwood_ratio(ft)*dc%hite))
       ...
       dc%laimemory = (1.0_r8/(1.0_r8 + pftcon%froot_leaf(ft) + &
            EDpftcon%sapwood_ratio(ft)*dc%hite))*dc%balive
  ...
  ! ============================================================================
  subroutine cwd_out( currentPatch )

    use clmtype,     only : pvs, cps, cws
