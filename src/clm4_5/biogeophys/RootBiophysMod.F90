...
!-------------------------------------------------------------------------------------- 
   subroutine init_vegrootfr(bounds, nlevsoi, nlevgrnd, pcolumn, ivt, zi, rootfr)
   !
   !DESCRIPTION
   ...
   integer,           intent(in) :: pcolumn(bounds%begp: )   !
   integer,           intent(in) :: ivt(bounds%begp: )       !
   ...
   SHR_ASSERT_ALL((ubound(ivt)    == (/bounds%endp/)), errMsg(__FILE__, __LINE__))
   SHR_ASSERT_ALL((ubound(zi)    == (/bounds%endc, nlevgrnd/)), errMsg(__FILE__, __LINE__))
   ...
   case (zeng_2001_root)
      rootfr(bounds%begp:bounds%endp, 1 : nlevsoi) = zeng2001_rootfr(bounds, nlevsoi, &
        pcolumn(bounds%begp:bounds%endp),     &
        ivt(bounds%begp:bounds%endp),         &
        zi(bounds%begc:bounds%endc,0:nlevsoi))
...
!--------------------------------------------------------------------------------------   
   function zeng2001_rootfr(bounds, ubj, pcolumn, ivt, zi)result(rootfr)
   !
   ! DESCRIPTION
   ...
   do p = bounds%begp,bounds%endp   
      if (ivt(p) /= noveg) then
         ...
         rootfr(p,ubj) = .5_r8*( exp(-roota_par(ivt(p)) * zi(c,ubj-1))  &
                                + exp(-rootb_par(ivt(p)) * zi(c,ubj-1)) )
      else
