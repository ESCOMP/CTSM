module EDPhotosynthesisMod
  ...
  ! This code is equivalent to the 'photosynthesis' subroutine in CanopyFluxesMod.
...
subroutine Photosynthesis_ED (bounds, fn, filterp, esat_tv, eair, oair, cair, &
     ...
     ! !USES:
     use clmtype               , only : pft, ces, pes, pps, pcf, pftcon, namep
     ...
     use EDClmtype             , only : EDpft, EDpcf, EDpftcon
...
pcolumn   => pft%column    , &     
ED_patch  => EDpft%ED_patch, &  ! does this 'p' have any vegetation associated with it?
ivt       => pft%itype     , &  ! pft vegetation type
t_soisno  => ces%t_soisno  , &  ! soil temperature (Kelvin)
t_veg     => pes%t_veg     , &  ! vegetation temperature (Kelvin)
tgcm      => pes%thm       , &  ! air temperature at agcm reference height (kelvin)
elai      => pps%elai      , &  ! exposed leaf area index
GCcanopy  => EDpcf%GCcanopy, &
...
lmrcanopy => pcf%lmrcanopy , &  ! Output: [real(r8) (:,:)]    !  canopy scale leaf maintenance respiration umol CO2 /m**2/ s
forc_pbot => a2l_downscaled_col%forc_pbot  , &! atmospheric pressure (Pa)
...
BB_slope  => EDpftcon%BB_slope               &  ! slope of BB relationship
)
!set timestep
 dtime = get_step_size()
...
if(ED_patch(p) == 1)then
  ...
  do FT = 1,numpft_ed
     ...
     !mbb(FT) = -11.0_r8* pftcon%wood_density(FT)+8.98_r8
  end do
                          ...
                          if (nint(c3psn(FT)) == 1)then
                             ...
                             if(sunsha == 1)then !sunlit
                                if((currentPatch%ed_laisun_z(cl,ft,iv) * currentPatch%canopy_area_profile(cl,ft,iv)) >  &
                                   ...
                                   aj = aj / (currentPatch%ed_laisun_z(cl,ft,iv)* currentPatch%canopy_area_profile(cl,ft,iv))
                                else
                                   aj = 0._r8
                                end if
                             else
                                aj = qe(ps) * currentPatch%ed_parsha_z(cl,ft,iv) * 4.6_r8
                                aj = aj / (currentPatch%ed_laisha_z(cl,ft,iv)* currentPatch%canopy_area_profile(cl,ft,iv))         
                             end if
  ...
  if(currentPatch%countcohorts > 0.0)then  !avoid errors caused by empty patches 
     ...
     do while (associated(currentCohort)) ! Cohort loop
        ...
        if(currentCohort%n > 0._r8)then   
           ...
           leaf_frac = 1.0_r8/(currentCohort%canopy_trim + EDpftcon%sapwood_ratio(currentCohort%pft) * &
                currentCohort%hite + pftcon%froot_leaf(currentCohort%pft))
           currentCohort%bsw = EDpftcon%sapwood_ratio(currentCohort%pft) * currentCohort%hite * (currentCohort%balive + &
           ...
           do j = 1,nlevsoi
              ...
              currentCohort%froot_mr = currentCohort%froot_mr + (1.0_r8 - coarse_wood_frac) *currentCohort%br*br*tcsoi* &
                   currentPatch%rootfr_ft(ft,j)/leafcn(currentCohort%pft) !fine root respn. 
              ! convert from gC/indiv/s-1 to kgC/indiv/s-1
