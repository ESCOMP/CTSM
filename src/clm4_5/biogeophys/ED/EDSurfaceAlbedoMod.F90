...
contains
   ...
   subroutine ED_Norman_Radiation (bounds, filter_vegsol, num_vegsol, coszen)
     ...
     !
     ! !USES:
     use clmtype,    only : pftcon, pft, cps, pps, pes
...
!-----------------------------------------------------------------------
   ...
   fabd_sun_z   =>    pps%fabd_sun_z    , & ! Output: [real(r8) (:,:)]  absorbed sunlit leaf direct  
                                            ! PAR (per unit lai+sai) for each canopy layer
   fabd_sha_z   =>    pps%fabd_sha_z    , & ! Output: [real(r8) (:,:)]  absorbed shaded leaf direct  
                                            ! PAR (per unit lai+sai) for each canopy layer
   fabi_sun_z   =>    pps%fabi_sun_z    , & ! Output: [real(r8) (:,:)]  absorbed sunlit leaf diffuse 
                                            ! PAR (per unit lai+sai) for each canopy layer
   fabi_sha_z   =>    pps%fabi_sha_z    , & ! Output: [real(r8) (:,:)]  absorbed shaded leaf diffuse 
                                            ! PAR (per unit lai+sai) for each canopy layer
               ...
               do ib = 1,numrad
                  ...
                  if(radtype == 1)then
                     ...
                     if( abs(currentPatch%sabs_dif(ib)-(currentPatch%tr_soil_dif(ib)*(1.0_r8-albgri(c,ib)))) > 0.0001)then
                        write(iulog,*)'dif ground absorption error',p,g,currentPatch%sabs_dif(ib),(currentPatch%tr_soil_dif(ib)* &
