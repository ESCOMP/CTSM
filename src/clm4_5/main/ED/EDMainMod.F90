  ...
  ! ============================================================================
    ...
    do g = bounds%begg,bounds%endg
       ...
       if(currentSite%istheresoil == 1)then
          call ecosystem_dynamics(currentSite) ! where most things happen
       endif
    enddo

    call ed_update_sites( bounds, gridCellEdState ) ! updates site & patch information
    call clm_ed_link( bounds, gridCellEdState )     ! link to CLM structures
...
!-------------------------------------------------------------------------------!
  ...
  ! ============================================================================
    ...
    call phenology(site_in%clmgcell)
    call fire_model(site_in)
    call disturbance_rates(site_in)         ! Calculate disturbance and mortality based on previous timestep vegetation.
    call integrate_state_variables(site_in) ! Integrate state variables from annual rates to daily timestep
    ...
    do while (associated(currentPatch))                 
       call recruitment(0,currentPatch)                ! adds small cohort of each PFT
    ...
    currentPatch => site_in%oldest_patch
    do while (associated(currentPatch))
       call terminate_cohorts(currentPatch)       ! kills cohorts that are too small
       call sort_cohorts(currentPatch)            ! puts cohorts in right order
       call fuse_cohorts(currentPatch)            ! fuses similar cohorts
       currentPatch => currentPatch%younger
    enddo
    ...
    !*******************************************************************************************************/
    call spawn_patches(site_in)       !make new patches from disturbed land
   
    call total_balance_check(site_in,3)
    call fuse_patches(site_in)        !fuse on the spawned patches.
   
    call total_balance_check(site_in,4)
    call terminate_patches(site_in)   !kill patches that are too small
...
!-------------------------------------------------------------------------------!
  ...
  subroutine  integrate_state_variables(site_in)
    ...
    do while(associated(currentPatch))
       ...
       call non_canopy_derivs(currentPatch)
