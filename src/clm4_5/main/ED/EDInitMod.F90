...
contains
  ...
  subroutine ed_init( bounds )
     ...
     if ( .not. is_restart() ) then
        ...
        call clm_ed_link( bounds, gridCellEdState )
  ...
  subroutine init_cohorts( patch_in )
     ...
     do pft =  1,2 !FIX(RF,032414) - turning off veg dynamics
        ...
        dc%pft                        = pft
        dc%n                          = EDpftcon%initd(pft) * patch_in%area
        dc%hite                       = EDpftcon%hgt_min(pft)
        dc%dbh                        = Dbh(dc) + 0.0001_r8*pft     !seperate out PFTs a little bit...
        dc%canopy_trim                = 1.0_r8
        dc%bdead                      = Bdead(dc)
        dc%balive                     = Bleaf(dc)*(1.0_r8 + pftcon%froot_leaf(pft) +EDpftcon%sapwood_ratio(dc%pft)*dc%hite)
        dc%b                          = dc%balive + dc%bdead

        if( pftcon%evergreen(pft)     == 1) then
           dc%bstore                  = Bleaf(dc) * EDpftcon%cushion(pft)
           dc%laimemory               = 0._r8
           cstatus                    = 2
        endif

        if( pftcon%season_decid(pft)  == 1 ) then !for dorment places
           dc%bstore                  = Bleaf(dc) * EDpftcon%cushion(pft) !stored carbon in new seedlings.
