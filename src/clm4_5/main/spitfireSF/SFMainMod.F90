module SFMainMod
  ...
  use clmtype               , only : pvs, pcf, pftcon
...
contains
  ...
  subroutine fire_model( site_in )
    ...
    if(use_ed_spit_fire.and.temporary_SF_switch==1)then
       call fire_danger_index(site_in)
       call wind_effect(site_in)
    ...
    !*****************************************************************
    subroutine  fire_danger_index ( currentSite )
    !*****************************************************************
    ...
    real(r8), pointer :: t_veg24(:) ! note - this is a p level temperature, which probably
                                    ! won't have much inpact, unless we decide
                                    ! to ever calculated the NI for each patch.  
    real(r8), pointer :: prec24(:)  ! note - this is a p level precip, which probably won't have much inpact, unless we decide
                                    ! to ever calculated the NI for each patch.  
    real(r8), pointer :: rh24(:)    ! note - this is a p level RH, which probably won't have much inpact, unless we decide
                                    ! to ever calculated the NI for each patch.  
    ...
    prec24           => EDpcf%prec24        ! Input:  [real(r8) (:)]  avg pft rainfall for last 24 hrs    
    rh24             => EDpcf%rh24          ! Input:  [real(r8) (:)]  avg pft relative humidity for last 24 hrs    
  ...
  !*****************************************************************
  subroutine  wind_effect ( currentSite )
  !*****************************************************************.
    ...
    wind24         => EDpcf%wind24  ! Input:  [real(r8) (:)]  avg pft windspeed (m/s)
       ...
       if (currentPatch%fire == 1) then
          ...
          do while(associated(currentCohort))  
             ...
             if (pftcon%woody(currentCohort%pft) == 1) then !trees only
                ...
                if (currentPatch%SH < (currentCohort%hite-currentCohort%hite*EDpftcon%crown(currentCohort%pft))) then 
                   currentCohort%cfa = 0.0_r8
                else
                   ...
                   if ((currentCohort%hite > 0.0_r8).and.(currentPatch%SH >=  &
                        (currentCohort%hite-currentCohort%hite*EDpftcon%crown(currentCohort%pft)))) then 

                           currentCohort%cfa =  (currentPatch%SH-currentCohort%hite* &
                                EDpftcon%crown(currentCohort%pft))/(currentCohort%hite-currentCohort%hite* &
       ...
       if (currentPatch%fire == 1) then
          ...
          do while(associated(currentCohort))  
             if (pftcon%woody(currentCohort%pft) == 1) then !trees only
                ...
                bt = EDpftcon%bark_scaler(currentCohort%pft)*currentCohort%dbh ! bark thickness. 
       ...
       if (currentPatch%fire == 1) then 
          ...
          do while(associated(currentCohort))  
             ...
             if (pftcon%woody(currentCohort%pft) == 1) then
                ...
                currentCohort%crownfire_mort = EDpftcon%crown_kill(currentCohort%pft)*currentCohort%cfa**3.0_r8
