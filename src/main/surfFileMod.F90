...
! !USES:
    ...
    use clm_varctl  , only : fsurdat, allocate_all_vegpfts
...
!-----------------------------------------------------------------------
    ...
    if (masterproc) then
       ...
       if (fsurdat == ' ') then
          write(6,*)'fsurdat must be specified'; call endrun()
       else
    ...
    ! Obtain surface dataset grid info
...
!
! !INTERFACE:
  subroutine surfrd_get_grid(ncid)
!
! !DESCRIPTION:
...
! !USES:
    ...
    use clm_varsur, only : lsmedge, numlon, longxy, latixy, lonw, lats, area, &
	                   landmask, landfrac 
...
#endif
    character(len=32) :: subname = 'surfrd_gridinfo'     ! subroutine name
!-----------------------------------------------------------------------
...
#if (defined OFFLINE)
       if (lsmedge(1) > 1.e35_r8) then    ! global grid without use of lsmedge
...
! !LOCAL VARIABLES:
    ...
    character(len=32) :: subname = 'surfrd_get_wtxy_special'  ! subroutine name
!-----------------------------------------------------------------------
...
!
! !USES:
    use clm_varsur, only : latixy, landmask, numlon, all_pfts_on_srfdat	
...
#endif
    character(len=32) :: subname = 'surfrd_get_wtxy_rank'  ! subroutine name
!-----------------------------------------------------------------------
       ...
       if (.not. all_pfts_on_srfdat) then
          ...
          do j = 1,lsmlat
             do i = 1,numlon(j)
                ...
                if (sumvec(i,j) > 1.e-04_r8 .and. landmask(i,j) == 1) then
       ...
       do i = 1,numlon(j)
          if (landmask(i,j) == 1) then
             ...
             do m = 1, maxpatch_pft
                ...
                if (veg(i,j,m) == 7 .and. abs(latixy(i,j)) < 23.5_r8) veg(i,j,m) = 6
                ...
                ! tropical deciduous broadleaf shrub type defined.
                 ...
                 if (veg(i,j,m) == 10 .and. abs(latixy(i,j)) < 23.5_r8) veg(i,j,m) = 6
#endif
             end do
    ...
    do j=1,lsmlat
       do i=1,numlon(j)
          if (sumvec(i,j) > 1.e-06_r8 .and. landmask(i,j)==1) then
             found = .true.
             iindx = i
             jindx = j
             exit
...
!
! !USES:
    use clm_varsur, only : latixy, landmask, numlon
!
! !ARGUMENTS:
    implicit none
    include 'netcdf.inc'
    ...
    do j = 1,lsmlat
       do i = 1,numlon(j)
          if (landmask(i,j) == 1) then
             ...
             mp8 = 8
             mp11 = 11
             if (abs(latixy(i,j)) < 23.5_r8 .and. wtxy(i,j,mp8) > 0._r8) then
             	 if (masterproc) then
                   write(6,*)'surfFileMod warning: reassigning temperate tree -> tropical tree'
                   write(6,*)'i,j,lat,veg7wt,veg6wt,type'
                   write(6,*) i,j,latixy(i,j),wtxy(i,j,mp8),wtxy(i,j,mp7),veg(i,j,mp8)
                end if
             	 wtxy(i,j,mp7) = wtxy(i,j,mp7) + wtxy(i,j,mp8)
                wtxy(i,j,mp8) = 0._r8
             	 if (masterproc) then
                   write(6,*) i,j,latixy(i,j),wtxy(i,j,mp8),wtxy(i,j,mp7)
                end if
             end if
             ...
             if (abs(latixy(i,j)) < 23.5_r8 .and. wtxy(i,j,mp11) > 0._r8) then
             	 if (masterproc) then
                   write(6,*)'surfFileMod warning: reassigning temperate shrub -> tropical tree'
                   write(6,*)'i,j,lat,veg10wt,veg6wt,type'
                   write(6,*) i,j,latixy(i,j),wtxy(i,j,mp11),wtxy(i,j,mp7),veg(i,j,mp11)
                end if
             	 wtxy(i,j,mp7) = wtxy(i,j,mp7) + wtxy(i,j,mp11)
                wtxy(i,j,mp11) = 0._r8
             	 if (masterproc) then
                   write(6,*) i,j,latixy(i,j),wtxy(i,j,mp11),wtxy(i,j,mp7)
                end if
             end if
#endif
          end if
