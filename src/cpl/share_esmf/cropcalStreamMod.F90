...
#include "shr_assert.h"
  ...
  use pftconMod        , only : num_prognostic_cfts_possible
...
!==============================================================================
  ...
  subroutine cropcal_init(bounds)
    ...
    allocate(stream_varnames_sdate(num_prognostic_cfts_possible))
    allocate(stream_varnames_cultivar_gdds(num_prognostic_cfts_possible))
    allocate(stream_varnames_gdd20_baseline(num_prognostic_cfts_possible))
    allocate(stream_varnames_gdd20_season_enddate(num_prognostic_cfts_possible))
    do n = 1,num_prognostic_cfts_possible
  ...
  subroutine cropcal_interp(bounds, num_pcropp, filter_pcropp, init, crop_inst)
    ...
    allocate(dataptr2d_swindow_start(begg:endg, num_prognostic_cfts_possible))
    dataptr2d_swindow_start(begg:endg,:) = -1._r8
    allocate(dataptr2d_swindow_end  (begg:endg, num_prognostic_cfts_possible))
    ...
    allocate(dataptr2d_cultivar_gdds(begg:endg, num_prognostic_cfts_possible))
    if (use_cropcal_rx_cultivar_gdds) then
       ...
       do fp = 1, num_pcropp
          ...
          if (is_prognostic_crop(ivt)) then
             ...
             if (n > num_prognostic_cfts_possible) then
                 write(iulog,'(a,i0,a,i0,a)') 'n (',n,') > ncft (',num_prognostic_cfts_possible,')'
                 call ESMF_Finalize(endflag=ESMF_END_ABORT)
             end if
   ...
   allocate(dataptr2d_gdd20_baseline(begg:endg, num_prognostic_cfts_possible))
   if (adapt_cropcal_rx_cultivar_gdds) then
  ...
  allocate(dataptr2d_gdd20_season_start(begg:endg, num_prognostic_cfts_possible))
  dataptr2d_gdd20_season_start(begg:endg,:) = -1._r8
  allocate(dataptr2d_gdd20_season_end  (begg:endg, num_prognostic_cfts_possible))
