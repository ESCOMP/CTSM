...
!===============================================================================
  ...
  subroutine lnd_register_esmf(comp, rc)
    ...
    call ESMF_GridCompSetEntryPoint(comp, ESMF_METHOD_INITIALIZE, lnd_init_esmf, rc=rc)
    if (rc /= ESMF_SUCCESS) call ESMF_Finalize(rc=rc, endflag=ESMF_END_ABORT)

    call ESMF_GridCompSetEntryPoint(comp, ESMF_METHOD_RUN, lnd_run_esmf, rc=rc)
    if (rc /= ESMF_SUCCESS) call ESMF_Finalize(rc=rc, endflag=ESMF_END_ABORT)

    call ESMF_GridCompSetEntryPoint(comp, ESMF_METHOD_FINALIZE, lnd_final_esmf, rc=rc)
    if (rc /= ESMF_SUCCESS) call ESMF_Finalize(rc=rc, endflag=ESMF_END_ABORT)
  ...
  subroutine lnd_init_esmf(comp, import_state, export_state, EClock, rc)
    ...
    use clm_varctl       , only : inst_suffix, inst_name
    ...
    ! TODO (mvertens, 2019-01-28): how is the clock set?
    ...
    ! TODO (mvertens, 2019-01-28): need to set start_type - what to do about restarts?
   ...
   call export_fields(gcomp, bounds, water_inst%waterlnd2atmbulk_inst, &
    ...
    ! TODO(mvertens, 2019-01-29): what do we do abou nextsw_cday??

    call export_fields(gcomp, bounds, water_inst%waterlnd2atmbulk_inst, lnd2atm_inst, rc)
    if (shr_nuopc_methods_ChkErr(rc,__LINE__,u_FILE_u)) return
