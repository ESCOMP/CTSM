...
!===============================================================================
  ...
  subroutine state_getscalar(state, scalar_id, scalar_value, flds_scalar_name, flds_scalar_num, rc)
    ...
    call ESMF_LogWrite(subname//'after VMGetCurrent', ESMF_LOGMSG_INFO)
    call ESMF_VMGet(vm, localPet=mytask, rc=rc)
    if (chkerr(rc,__LINE__,u_FILE_u)) return
    call ESMF_LogWrite(subname//'after VMGet', ESMF_LOGMSG_INFO)

    call ESMF_LogWrite(subname//'before ESMF_StateGet', ESMF_LOGMSG_INFO)
    call ESMF_LogWrite(subname//'or field is '//trim(flds_scalar_name), ESMF_LOGMSG_INFO)
    
    call ESMF_StateGet(State, itemName=trim(flds_scalar_name), field=field, rc=rc)
    if (chkerr(rc,__LINE__,u_FILE_u)) return
    call ESMF_LogWrite(subname//'after ESMF_StateGet', ESMF_LOGMSG_INFO)
...
!================================================================================
  ...
  subroutine state_setscalar(scalar_value, scalar_id, State, flds_scalar_name, flds_scalar_num,  rc)
    ...
    print *, trim(flds_scalar_name)
    
    call ESMF_LogWrite(subname//'before ESMF_StateSet', ESMF_LOGMSG_INFO)
    call ESMF_LogWrite(subname//'itemName:'//trim(flds_scalar_name), ESMF_LOGMSG_INFO)
    !call ESMF_LogWrite(subname//'State: '//State, ESMF_LOGMSG_INFO)
    !print *, 'itemName:', trim(flds_scalar_name)
    !print *, 'lfield:', lfield
    ...
    if (mytask == 0) then
       call ESMF_LogWrite(subname//'before FieldGet', ESMF_LOGMSG_INFO)
       !print *, 'lfield:', lfield
       !print *, 'this farrayptr is ', farrayptr
