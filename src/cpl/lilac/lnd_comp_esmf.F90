module lnd_comp_esmf
    ...
    use lnd_shr_methods   , only : log_clock_advance
    !use lnd_import_export , only : realize_fields
    ...
    !NS: from https://github.com/amangupta2/CAM_Trunk/blob/33b0fd4bc2c3b945b93655ee8b5e20f1acf5625b/components/cam/src/cpl/nuopc/atm_comp_nuopc.F90
    !--------------------------------------------------------------------------
    ! Private module data
    !--------------------------------------------------------------------------
    ...
    !character(*)    ,parameter      :: modName =  "ctsm_lilac"
...
!===============================================================================
    ...
    subroutine lnd_init(comp, import_state, export_state, clock, rc)
        ...
        !-----------------------------------------------------------------------
        ! NS : from
        ! https://github.com/amangupta2/CAM_Trunk/blob/33b0fd4bc2c3b945b93655ee8b5e20f1acf5625b/components/cam/src/cpl/nuopc/atm_comp_nuopc.F90
        ...
        !! FIXME(ns, 2019-07-29)
        !! glc_nec should be set from driver or higher level lilac?
       ...
       !      orb_obliqr=obliqr, orb_lambm0=orb_lambm0, orb_mvelpp=orb_mvelpp, masterproc)
        ...
        !call State_SetScalar(dble(ldomain%ni), flds_scalar_index_nx, export_state, &
    ...
    call ESMF_LogWrite(subname//' CTSM INITIALIZATION DONE SUCCESSFULLY!!!! ', ESMF_LOGMSG_INFO)
      ...
      !---------------------------------------------------------------------------
        ...
        !    flds_scalar_name, flds_scalar_num, rc)
        !call ESMF_LogWrite(subname//'after state_getscalar for determining nextsw_cday', ESMF_LOGMSG_INFO)
        !call set_nextsw_cday( nextsw_cday )
        !call ESMF_LogWrite(subname//'settitng nextsw_cday', ESMF_LOGMSG_INFO)
        ...
        !call import_fields( import_state, bounds, glc_present, rof_prognostic, atm2lnd_inst, glc2lnd_inst, water_inst%wateratm2lndbulk_inst, rc )
