...
contains
  ...
  subroutine create_cohort(pft,nn,hite,dbh,balive,bdead,bstore,laimemory,status,ctrim,clayer,patchptr)
    ...
    if(new_cohort%patchptr%siteptr%status==2.and.ecophyscon%season_decid(pft) == 1)then
      new_cohort%laimemory = 0.0_r8
    endif
    if(new_cohort%patchptr%siteptr%dstatus==2.and.ecophyscon%stress_decid(pft) == 1)then
      new_cohort%laimemory = 0.0_r8
    endif
...
!-------------------------------------------------------------------------------------!
  ...
  ! ============================================================================
    ...
    leaves_off_switch = 0
    if(currentcohort%status_coh == 1.and.ecophyscon%stress_decid(ft) == 1.and.currentcohort%siteptr%dstatus==1)then !no leaves 
      leaves_off_switch = 1 !drought decid
    endif
    if(currentcohort%status_coh == 1.and.ecophyscon%season_decid(ft) == 1.and.currentcohort%siteptr%status==1)then !no leaves
