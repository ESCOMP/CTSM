    ...
    associate(                                                            &
      ...
      if ( use_crop )then
         ...
         if (use_grainproduct) then !added leaf if statement to create leaf_mr_patch if use_livestemproduct MWGraham
            do fp = 1,num_soilp
               p = filter_soilp(fp)
               iso_cnveg_cf%leafc_to_cropprodc_patch(p) = iso_cnveg_cf%leafc_to_litter_patch(p)
               iso_cnveg_cf%leaf_mr_patch(p) = iso_cnveg_cf%leaf_xsmr_patch(p) + iso_cnveg_cf%leaf_curmr_patch(p)
            end do
         endif
      end if
