...
! ! available to the plant for grwoth, and the C spent on obtaining it. 
...
! !PUBLIC MEMBER FUNCTIONS:
  ...
  integer :: RT_METHOD = 2
  integer :: COST_METHOD = 2 !new way of doing the N uptake resistances. see teamwork thread on over-cheap uptake in N resistors. 
!
!-----------------------------------------------------------------------
 contains
!-----------------------------------------------------------------------
...
! Set local pointers
  associate(                                                                              &
         ...
         leafc_storage_xfer_acc => cnveg_carbonstate_inst%leafc_storage_xfer_acc_patch  , & ! Output: [real(r8) (:)   ]  Accmulated leaf C transfer (gC/m2)
         storage_cdemand        => cnveg_carbonstate_inst%storage_cdemand_patch         , & ! Output: [real(r8) (:)   ]  C use from the C storage pool
...
!-----------------------------------------------------------------------
  subroutine CNFUN(bounds,num_soilc, filter_soilc,num_soilp,filter_soilp,waterstate_inst                 ,&
...
! !USES:
   ...
   use clm_varcon      , only : secspday, smallValue, fun_period, tfrz, dzsoi_decomp
...
!-------------------------------------------------------------------------------
  ...
  logical             :: l_fun_passive = .FALSE.          ! TRUE if passive uptake  is allowed
  ...
  logical             :: cost_minimized  = .FALSE.        ! Only choose the least expensive uptake option. 
!-------------------------------------------------------------------------------
! Inteter parameters
!-------------------------------------------------------------------------------
...
!if (.not. use_nitrif_denitrif) then
  integer,  parameter :: ncost4          = 4             ! Number of N transport pathways
  integer,  parameter :: icostActiveSmin = 3             ! Process number for mycorrhizal uptake.
  integer,  parameter :: icostNonmyc     = 4             ! Process number for nonmyc uptake.
!else  !----------NITRIF_DENITRIF-------------! 
  integer,  parameter :: ncost6          = 6             ! Number of N transport pathways
...
!-------------------------------------------------------------------------------------------------------------------
  ...
  real(r8)  :: plant_ndemand_retrans_step(bounds%begp:bounds%endp,1:nstp)! the N demand pool for offset          (gN/m2)
  real(r8)  :: leafn_to_litter_fun(bounds%begp:bounds%endp)              ! A temporary variable for retrans      (gN/m2)
  ...
  real(r8)  :: litterfall_step(bounds%begp:bounds%endp,1:nstp)           ! N loss based on the leafc to litter   (gN/m2)
  real(r8)  :: litterfall_retrans_step(bounds%begp:bounds%endp,1:nstp)   ! N loss based on the leafc to litter   (gN/m2)
  ...
  real(r8)  :: npp_remaining_temp(bounds%begp:bounds%endp,1:nstp)        ! A temporary variable for npp_remaining(gC/m2)
  real(r8)  :: npp_remaining_retrans(bounds%begp:bounds%endp,1:nstp)     ! A temporary variable for npp_remaining(gC/m2)
  real(r8)  :: npp_remaining_total(bounds%begp:bounds%endp)              ! A temporary variable for npp_remaining(gC/m2)
  ...
  real(r8)  :: n_fix_temp1(bounds%begp:bounds%endp,1:nlevdecomp)         ! Fixation in each soil layer           (gN/m2)
  real(r8)  :: n_fix_temp2(bounds%begp:bounds%endp,1:nlevdecomp)         ! Fixation in each soil layer           (gN/m2)
  ...
  real(r8)  :: n_fix_retrans(bounds%begp:bounds%endp,1:nstp)             ! N acquired by fixation                (gN/m2)
  real(r8)  :: n_fix_retrans_total(bounds%begp:bounds%endp)              ! N acquired by fixation                (gN/m2)
  real(r8)  :: n_fix_vr(bounds%begp:bounds%endp,1:nlevdecomp)            ! Fixation in each soil layer           (gN/m2)
  ...
  real(r8)  :: npp_fix_retrans(bounds%begp:bounds%endp,1:nstp)           ! Amount of NPP used by fixation        (gC/m2)
  real(r8)  :: npp_fix_retrans_total(bounds%begp:bounds%endp)            ! Amount of NPP used by fixation        (gC/m2)
  ...
  real(r8)  :: n_retrans_retrans(bounds%begp:bounds%endp,1:nstp)         ! N acquired by retranslocation         (gN/m2)
  real(r8)  :: n_retrans_retrans_total(bounds%begp:bounds%endp)          ! N acquired by retranslocation         (gN/m2)
  ...
  real(r8)  :: npp_retrans_retrans(bounds%begp:bounds%endp,1:nstp)       ! NPP used for the extraction           (gC/m2)
  real(r8)  :: npp_retrans_retrans_total(bounds%begp:bounds%endp)        ! NPP used for the extraction           (gC/m2)
  real(r8)  :: nt_uptake(bounds%begp:bounds%endp,1:nstp)                 ! N uptake from retrans, active, and fix(gN/m2)
  real(r8)  :: nt_uptake_retrans(bounds%begp:bounds%endp,1:nstp)         ! N uptake from retrans, active, and fix(gN/m2)
  real(r8)  :: npp_uptake(bounds%begp:bounds%endp,1:nstp)                ! NPP used by the uptakes               (gC/m2)
  real(r8)  :: npp_uptake_retrans(bounds%begp:bounds%endp,1:nstp)        ! NPP used by the uptakes               (gC/m2)

  !------.not.use_nitrif_denitrif------!

  real(r8)  :: sminn_diff                                                       ! A temporary limit for N uptake                  (gN/m2) 
  real(r8)  :: active_limit1                                                    ! A temporary limit for N uptake                  (gN/m2)
  real(r8)  :: costNon(1:nlevdecomp,ncost4)                                     ! Cost of N via each process                      (gC/gN)
  real(r8)  :: cost_active(bounds%begp:bounds%endp,1:nlevdecomp)                ! cost of mycorrhizal                             (gC/gN)
  real(r8)  :: cost_nonmyc(bounds%begp:bounds%endp,1:nlevdecomp)                ! cost of nonmyc                                  (gC/gN)
  real(r8)  :: sminn_conc(bounds%begc:bounds%endc,1:nlevdecomp)                 ! Concentration of N in soil water              (gN/gH2O)
  real(r8)  :: sminn_conc_step(bounds%begp:bounds%endp,1:nlevdecomp,1:nstp)     ! A temporary variable for soil mineral N       (gN/gH2O)
  real(r8)  :: sminn_layer(bounds%begc:bounds%endc,1:nlevdecomp)                ! Available mineral N in soil water               (gN/m2)
  real(r8)  :: sminn_layer_step(bounds%begp:bounds%endp,1:nlevdecomp,1:nstp)    ! A temporary variable for soil mineral N         (gN/m2)
  real(r8)  :: sminn_uptake(bounds%begp:bounds%endp,1:nlevdecomp,1:nstp)        ! A temporary variable for soil mineral N       (gN/m2/s)
  real(r8)  :: active_uptake1(bounds%begp:bounds%endp,1:nlevdecomp)             ! mycorrhizal uptake                              (gN/m2) 
  real(r8)  :: active_uptake2(bounds%begp:bounds%endp,1:nlevdecomp)             ! mycorrhizal uptake                              (gN/m2)
  real(r8)  :: nonmyc_uptake1(bounds%begp:bounds%endp,1:nlevdecomp)             ! non-mycorrhizal uptake                          (gN/m2) 
  real(r8)  :: nonmyc_uptake2(bounds%begp:bounds%endp,1:nlevdecomp)             ! non-mycorrhizal uptake                          (gN/m2)
  real(r8)  :: n_am_acc(bounds%begp:bounds%endp)                                ! A temporary variable for AM  uptake             (gN/m2)
  real(r8)  :: n_ecm_acc(bounds%begp:bounds%endp)                               ! A temporary variable for ECM uptake             (gN/m2)
  real(r8)  :: n_am_retrans(bounds%begp:bounds%endp)                            ! A temporary variable for AM  uptake             (gN/m2)
  real(r8)  :: n_ecm_retrans(bounds%begp:bounds%endp)                           ! A temporary variable for ECM uptake             (gN/m2)
  real(r8)  :: n_active_acc(bounds%begp:bounds%endp,1:nstp)                     ! N acquired by mycorrhizal uptake                (gN/m2)
  real(r8)  :: n_nonmyc_acc(bounds%begp:bounds%endp,1:nstp)                     ! N acquired by nonmyc uptake                     (gN/m2)
  real(r8)  :: n_active_retrans(bounds%begp:bounds%endp,1:nstp)                 ! N acquired by mycorrhizal uptake                (gN/m2)
  real(r8)  :: n_nonmyc_retrans(bounds%begp:bounds%endp,1:nstp)                 ! N acquired by nonmyc uptake                     (gN/m2)
  real(r8)  :: n_active_acc_total(bounds%begp:bounds%endp)                      ! A temporary variable for mycorrhizal uptake     (gN/m2) 
  real(r8)  :: n_nonmyc_acc_total(bounds%begp:bounds%endp)                      ! A temporary variable for non-mycorrhizal uptake (gN/m2)
  real(r8)  :: n_active_retrans_total(bounds%begp:bounds%endp)                  ! A temporary variable for mycorrhizal uptake     (gN/m2)
  real(r8)  :: n_nonmyc_retrans_total(bounds%begp:bounds%endp)                  ! A temporary variable for non-mycorrhizal uptake (gN/m2)
  real(r8)  :: n_passive_vr(bounds%begp:bounds%endp,1:nlevdecomp)               ! Passive N uptake in each soil layer             (gN/m2)
  real(r8)  :: n_active_vr(bounds%begp:bounds%endp,1:nlevdecomp)                ! Mycorrhizal uptake in each soil layer           (gN/m2)
  real(r8)  :: n_nonmyc_vr(bounds%begp:bounds%endp,1:nlevdecomp)                ! Non-mycorrhizal uptake in each soil layer       (gN/m2)
  real(r8)  :: npp_active_acc(bounds%begp:bounds%endp,1:nstp)                   ! NPP used by mycorrhizal uptake                  (gC/m2)
  real(r8)  :: npp_nonmyc_acc(bounds%begp:bounds%endp,1:nstp)                   ! NPP used by nonmyc uptake                       (gC/m2)
  real(r8)  :: npp_active_acc_total(bounds%begp:bounds%endp)                    ! NPP used by mycorrhizal uptake                  (gC/m2)
  real(r8)  :: npp_nonmyc_acc_total(bounds%begp:bounds%endp)                    ! NPP used by nonmyc uptake                       (gC/m2)
  real(r8)  :: npp_active_retrans(bounds%begp:bounds%endp,1:nstp)               ! NPP used by mycorrhizal uptake                  (gC/m2)
  real(r8)  :: npp_nonmyc_retrans(bounds%begp:bounds%endp,1:nstp)               ! NPP used by nonmyc uptake                       (gC/m2)
  real(r8)  :: npp_active_retrans_total(bounds%begp:bounds%endp)                ! NPP used by mycorrhizal uptake                  (gC/m2)
  real(r8)  :: npp_nonmyc_retrans_total(bounds%begp:bounds%endp)                ! NPP used by nonmyc uptake                       (gC/m2)
  ...
  real(r8)  :: sminn_layer_step1(bounds%begp:bounds%endp,1:nlevdecomp,1:nstp)   ! A temporary variable for soil N                 (gN/m2)
  real(r8)  :: sminn_layer_step2(bounds%begp:bounds%endp,1:nlevdecomp,1:nstp)   ! A temporary variable for soil N                 (gN/m2)
...
! Added on Sep 22
  real(r8)  :: fno3(bounds%begc:bounds%endc,1:nlevdecomp)                       ! the fraction of NO3 in soil                     (unitless)
  real(r8)  :: fnh4(bounds%begc:bounds%endc,1:nlevdecomp)                       ! the fraction of NH4 in soil                     (unitless)
  real(r8)  :: ftotal(bounds%begc:bounds%endc,1:nlevdecomp)                     ! the total amount of NO3 and NH4                 (gN/m3/s) 
  ...
  real(r8)  ::                  sum_n_acquired(1:nlevdecomp)                           ! Sum N aquired from one unit of C (unitless)  
  real(r8)  ::                  carbon_for_allocation                                ! how much carbon is now reserved for allocation. gC/m2/s-1
  real(r8)  ::                  burned_off_carbon                                     ! carbon wasted by poor allocation algorithm. If this is too big, we need a better iteration. 
  real(r8)   ::                 temp_n_flux  
  real(r8)  :: mincost          ! minimum cost of nitrogen uptake for switch variant. 
  real(r8)  ::                  actual_leafcn                                      ! actual leaf C:N ratio. C/N
  ...
  real(r8) :: excess_carbon_acc ! excess accumulated over layers. WITHOUT GROWTH RESP
...
!-----------------------------------------------------------------------------------------------------
  associate(                                                                              &
         ...
         kr_resorb              => pftcon%kr_resorb                                     , & ! Input:   A retranslocation parameter
         ...
         smin_vr_nh4                  => soilbiogeochem_nitrogenstate_inst%smin_nh4_vr_col             , & ! Input:  [real(r8) (:,:) ]  (gN/m3) soil mineral NH4              
         smin_vr_no3                  => soilbiogeochem_nitrogenstate_inst%smin_no3_vr_col             , & ! Input:  [real(r8) (:,:) ]  (gN/m3) soil mineral NO3              
...
!-------------------------------------------------------------------------------
  ...
  costNit(:,:)                    = big_cost 
  cost_active_no3(:,:)            = 0._r8
  cost_active_nh4(:,:)            = 0._r8
  cost_nonmyc_no3(:,:)            = 0._r8
  cost_nonmyc_nh4(:,:)            = 0._r8
  sminn_no3_conc(:,:)             = 0._r8
  sminn_no3_conc_step(:,:,:)      = 0._r8
  sminn_no3_layer(:,:)            = 0._r8
  sminn_no3_layer_step(:,:,:)     = 0._r8
  sminn_no3_uptake(:,:,:)         = 0._r8
  sminn_nh4_conc(:,:)             = 0._r8
  sminn_nh4_conc_step(:,:,:)      = 0._r8
  sminn_nh4_layer(:,:)            = 0._r8
  sminn_nh4_layer_step(:,:,:)     = 0._r8
  sminn_nh4_uptake(:,:,:)         = 0._r8
  sminn_layer_step1(:,:,:)        = 0._r8
  sminn_layer_step2(:,:,:)        = 0._r8
! Added on Sep 22
  fno3(:,:)                       = 0._r8
  fnh4(:,:)                       = 0._r8
  ftotal(:,:)                     = 0._r8
...
!------------------------------------------------------------------------------------------------
  do fp = 1,num_soilp        ! PFT Starts
     ...
     if (plantN(p).gt.0._r8) then 
         plantCN(p) = c_allometry(p)/n_allometry(p) !changed RF. above code gives CN ratio too low. 
     else
         plantCN(p) = 0._r8 
     end if
   end do   ! PFT ends
...
! Changed on Sep 22
       ftotal(c,j)            = smin_no3_to_plant_vr(c,j) + smin_nh4_to_plant_vr(c,j)
       if (ftotal(c,j).gt.0._r8) then 
          fno3(c,j)           = smin_no3_to_plant_vr(c,j)/ftotal(c,j)       
          fnh4(c,j)           = smin_nh4_to_plant_vr(c,j)/ftotal(c,j)
       end if
! Change ends 
       ...
       if (h2osoi_liq(c,j) > smallValue) then
          sminn_no3_conc(c,j)  = sminn_no3_layer(c,j) / (h2osoi_liq(c,j) * 1000._r8) ! (gN/m2)/(gH2O/m2) (coverted from kg2g)
          sminn_nh4_conc(c,j)  = sminn_nh4_layer(c,j) / (h2osoi_liq(c,j) * 1000._r8) ! (gN/m2)/(gH2O/m2) (coverted from kg2g)
       else
  ...
  n_am_no3_acc(:)                 = 0._r8
  n_am_nh4_acc(:)                 = 0._r8
  n_am_no3_retrans(:)             = 0._r8
  n_am_nh4_retrans(:)             = 0._r8
  n_ecm_no3_acc(:)                = 0._r8
  n_ecm_nh4_acc(:)                = 0._r8
  n_ecm_no3_retrans(:)            = 0._r8
  n_ecm_nh4_retrans(:)            = 0._r8
  n_active_no3_acc(:,:)           = 0._r8   
  n_active_nh4_acc(:,:)           = 0._r8  
  n_active_no3_acc_total(:)       = 0._r8 
  n_active_nh4_acc_total(:)       = 0._r8
  n_active_no3_retrans(:,:)       = 0._r8
  n_active_nh4_retrans(:,:)       = 0._r8
  n_active_no3_retrans_total(:)   = 0._r8 
  n_active_nh4_retrans_total(:)   = 0._r8
  n_nonmyc_no3_acc(:,:)           = 0._r8  
  n_nonmyc_nh4_acc(:,:)           = 0._r8  
  n_nonmyc_no3_acc_total(:)       = 0._r8
  n_nonmyc_nh4_acc_total(:)       = 0._r8
  n_nonmyc_no3_retrans(:,:)       = 0._r8
  n_nonmyc_nh4_retrans(:,:)       = 0._r8
  n_nonmyc_no3_retrans_total(:)   = 0._r8
  n_nonmyc_nh4_retrans_total(:)   = 0._r8
  n_passive_no3_vr(:,:)           = 0._r8
  n_passive_nh4_vr(:,:)           = 0._r8
  n_fix_no3_vr(:,:)               = 0._r8
  n_fix_nh4_vr(:,:)               = 0._r8
  n_active_no3_vr(:,:)            = 0._r8
  n_nonmyc_no3_vr(:,:)            = 0._r8
  n_active_nh4_vr(:,:)            = 0._r8
  n_nonmyc_nh4_vr(:,:)            = 0._r8
  npp_active_no3_acc(:,:)         = 0._r8
  npp_active_nh4_acc(:,:)         = 0._r8
  npp_active_no3_retrans(:,:)     = 0._r8
  npp_active_nh4_retrans(:,:)     = 0._r8
  npp_active_no3_acc_total(:)     = 0._r8
  npp_active_nh4_acc_total(:)     = 0._r8
  npp_active_no3_retrans_total(:) = 0._r8
  npp_active_nh4_retrans_total(:) = 0._r8
  npp_nonmyc_no3_acc(:,:)         = 0._r8
  npp_nonmyc_nh4_acc(:,:)         = 0._r8
  npp_nonmyc_no3_retrans(:,:)     = 0._r8
  npp_nonmyc_nh4_retrans(:,:)     = 0._r8
  npp_nonmyc_no3_acc_total(:)     = 0._r8
  npp_nonmyc_nh4_acc_total(:)     = 0._r8
  npp_nonmyc_no3_retrans_total(:) = 0._r8
  npp_nonmyc_nh4_retrans_total(:) = 0._r8
...
! I have turned off this r etranslocation functionality for now. To be rolled back in to a new version later on once the rest of th
...
!------------------------------------------------------------------------------
      ...
      ! zero out all of the fluxes that get accumulated accross ISTP 
         ...
         active_no3_uptake1(:,:)     = 0._r8
         active_nh4_uptake1(:,:)     = 0._r8
         nonmyc_no3_uptake1(:,:)     = 0._r8
         nonmyc_nh4_uptake1(:,:)     = 0._r8
         ...
         if (plant_ndemand_pool_step(p,istp) .gt. 0._r8) then   ! plant_ndemand_pool_step > 0.0
            
            if (l_fun_fix) then
               do j = 1, nlevdecomp
                  tc_soisno(c,j)          = t_soisno(c,j)  -   tfrz
                  costNit(j,icostFix)     = fun_cost_fix(fixer,a_fix(ivt(p)),b_fix(ivt(p)),c_fix(ivt(p)) &
                                             ,big_cost,rootfr(p,j),s_fix(ivt(p)),tc_soisno(c,j))
               end do
            else
               costNit(:,icostFix)        = big_cost     
            end if
...
!--------------------------------------------------------------------------------
            if (l_fun_passive) then 
               do j= 1, nlevdecomp 
                  qflx_tran_veg_layer        = qflx_tran_veg(p) * rootfr(p,j)  * permyc(p,istp)           
                  sminn_no3_uptake(p,j,istp) = sminn_no3_conc_step(p,j,istp)   * (1000._r8 * qflx_tran_veg_layer)
                  sminn_nh4_uptake(p,j,istp) = sminn_nh4_conc_step(p,j,istp)   * (1000._r8 * qflx_tran_veg_layer)
                  sminn_no3_uptake(p,j,istp) = min(sminn_no3_uptake(p,j,istp), sminn_no3_layer_step(p,j,istp)/dt)
                  sminn_nh4_uptake(p,j,istp) = min(sminn_nh4_uptake(p,j,istp), sminn_nh4_layer_step(p,j,istp)/dt)
                  n_passive_step(p,istp)     = n_passive_step(p,istp) + (sminn_no3_uptake(p,j,istp) + &
                                               sminn_nh4_uptake(p,j,istp)) * dt
               end do  ! j 
            ...
            if (plant_ndemand_pool_step(p,istp) <= n_passive_step(p,istp)) then 
                ...
                do j = 1,  nlevdecomp
                      sminn_no3_uptake(p,j,istp)   = sminn_no3_uptake(p,j,istp) - &
                                                     (excess/dt) * sminn_no3_uptake(p,j,istp)/ (n_passive_step(p,istp)/dt)
                      sminn_nh4_uptake(p,j,istp)   = sminn_nh4_uptake(p,j,istp) - &
                                                     (excess/dt) * sminn_nh4_uptake(p,j,istp)/ (n_passive_step(p,istp)/dt)
                end do  ! j 
...
!----modfied on Sep 14th---!!!
            do j = 1,  nlevdecomp
                n_passive_no3_vr(p,j)        = n_passive_no3_vr(p,j) + sminn_no3_uptake(p,j,istp) * dt
                n_passive_nh4_vr(p,j)        = n_passive_nh4_vr(p,j) + sminn_nh4_uptake(p,j,istp) * dt
            end do
!----modfied on Sep 14th---!!!
...
!--------------------------------------------------------------------------------
            ...
            do j = 1, nlevdecomp
               sminn_no3_layer_step(p,j,istp)    = sminn_no3_layer_step(p,j,istp) - sminn_no3_uptake(p,j,istp) * dt
               sminn_nh4_layer_step(p,j,istp)    = sminn_nh4_layer_step(p,j,istp) - sminn_nh4_uptake(p,j,istp) * dt
               sminn_no3_layer_step(p,j,istp)    = max(sminn_no3_layer_step(p,j,istp),0._r8)
               sminn_nh4_layer_step(p,j,istp)    = max(sminn_nh4_layer_step(p,j,istp),0._r8)
               sminn_layer_step1(p,j,istp)       = sminn_no3_layer_step(p,j,istp) + sminn_nh4_layer_step(p,j,istp)
            end do
!--------------------------------------------------------------------------------
...
!--------------------------------------------------------------------------------
            do j = 1, nlevdecomp
               if (h2osoi_liq(c,j) > smallValue) then
                  sminn_no3_conc_step(p,j,istp)  = sminn_no3_layer_step(p,j,istp) / (h2osoi_liq(c,j) * 1000._r8)  ! gN/m2/gH2O/m2
                  sminn_nh4_conc_step(p,j,istp)  = sminn_nh4_layer_step(p,j,istp) / (h2osoi_liq(c,j) * 1000._r8)  ! gN/m2/gH2O/m2
               else
...
!--------------------------------------------------------------------------------
!         If passive uptake is insufficient, consider fixation, mycorrhizal 
...
!--------------------------------------------------------------------------------
         ...
         !------Mycorrhizal Uptake Cost-----------------!
            if (l_fun_active) then
               do j = 1,nlevdecomp
                  ...
                  costNit(j,icostActiveNO3)  = fun_cost_active(smin_vr_no3(c,j)            &
                                             ,big_cost,kc_active(p,istp),kn_active(p,istp)              &
                                             ,rootc_dens_step,rootfr(p,j),smallValue)
                  costNit(j,icostActiveNH4)  = fun_cost_active(smin_vr_no3(c,j)            &
                                             ,big_cost,kc_active(p,istp),kn_active(p,istp)              &
                                             ,rootc_dens_step,rootfr(p,j),smallValue)
               end do
            else
               costNit(:,icostActiveNO3)     = big_cost
               costNit(:,icostActiveNH4)     = big_cost
            end if      
         ...
         !------Non-mycorrhizal Uptake Cost-------------!
            if (l_fun_nonmyc) then
               do j = 1,nlevdecomp
                  rootc_dens_step             = rootc_dens(p,j)  *  permyc(p,istp)
                  costNit(j,icostnonmyc_no3)   = fun_cost_nonmyc(smin_vr_nh4(c,j)            &
                                                ,big_cost,kc_nonmyc(ivt(p)),kn_nonmyc(ivt(p))              &
                                                ,rootc_dens_step,rootfr(p,j),smallValue)
                  costNit(j,icostnonmyc_nh4)   = fun_cost_nonmyc(smin_vr_nh4(c,j)            &
                                                ,big_cost,kc_nonmyc(ivt(p)),kn_nonmyc(ivt(p))              &
                                                ,rootc_dens_step,rootfr(p,j),smallValue)
               end do
            else
               costNit(:,icostnonmyc_no3)      = big_cost  
               costNit(:,icostnonmyc_nh4)      = big_cost 
            end if
            cost_nonmyc_no3(p,1:nlevdecomp)   = costNit(:,icostnonmyc_no3)
            cost_nonmyc_nh4(p,1:nlevdecomp)   = costNit(:,icostnonmyc_nh4)
            

         !------Retranslocation Uptake Cost-------------!  
            if (l_fun_retrans) then
               if(tlai(p).gt.0._r8)then
               leaf_narea = leafn(p)/tlai(p)
               else
               leaf_narea = 0.00000001_r8 !no leaf area. 
               endif
               if (season_decid(ivt(p)) == 1._r8 .or. stress_decid(ivt(p)) == 1._r8) then
                  
                  if (offset_flag(p) == 1._r8) then
                  ! for deciduous trees, the whole leafn pool is available for retranslocation. 
                     costNit(:,icostRetrans) = fun_cost_retrans(big_cost,kr_resorb(ivt(p)),leaf_narea,smallValue)
                  else
                     costNit(:,icostRetrans) = big_cost 
                  end if
               else
                  ! for evergreen trees, only part of the leafn pool is available for retranslocation. 
                  costNit(:,icostRetrans)    = fun_cost_retrans(big_cost,kr_resorb(ivt(p)),leaf_narea,smallValue)
               end if
            else
               costNit(:,icostRetrans)       = big_cost
           ...
           ! costNit(:,icostRetrans)       = big_cost ! add this in as a test RF. REMOVE
            cost_retran(p,1:nlevdecomp)      = costNit(:,icostRetrans)
...
!-------------------------------------------------------------------------------          
            do j = 1, nlevdecomp
               ...
               cost_res_resis(p,j) = 1._r8/cost_active_no3(p,j) + 1._r8/cost_active_nh4(p,j) &
                              + 1._r8/cost_nonmyc_no3(p,j) + 1._r8/cost_nonmyc_nh4(p,j) & 
                              + 1._r8/cost_retran(p,j)     + 1._r8/cost_fix(p,j)                              
               
               ! Calculate npp allocation to pathways proportional to their exchange rate (N/C) 
               npp_frac_to_fixation(j)   = (1._r8/cost_fix(p,j))        / cost_res_resis(p,j)
               npp_frac_to_retrans(j)    = (1._r8/cost_retran(p,j))     / cost_res_resis(p,j)
               npp_frac_to_active_nh4(j) = (1._r8/cost_active_nh4(p,j)) / cost_res_resis(p,j)
               npp_frac_to_nonmyc_nh4(j) = (1._r8/cost_nonmyc_nh4(p,j)) / cost_res_resis(p,j)
               npp_frac_to_active_no3(j) = (1._r8/cost_active_no3(p,j)) / cost_res_resis(p,j)
               npp_frac_to_nonmyc_no3(j) = (1._r8/cost_nonmyc_no3(p,j)) / cost_res_resis(p,j)
               
                     
               ! Calculate hypothetical N uptake from each source                     
               n_exch_fixation(j)   = npp_frac_to_fixation(j)   / cost_fix(p,j)
               n_exch_retrans(j)    = npp_frac_to_retrans(j)    / cost_retran(p,j) 
               ...
               sum_n_acquired(j)        = n_exch_fixation(j) + n_exch_retrans(j) + n_exch_active_nh4(j) +&
                                          n_exch_nonmyc_nh4(j)+ n_exch_active_no3(j) + n_exch_nonmyc_no3(j)
               
               ! Effective cost of N uptake (C/N)                          
               cost_resis(p,j)     =  1.0_r8 / sum_n_acquired(j) 
...
!-------------------------------------------------------------------------------
             ...
             ! Remove C required to pair with N from passive uptake from the available pool. 
              npp_remaining(p,istp)  =   npp_remaining(p,istp) - n_passive_step(p,istp)*plantCN(p)
			     ...
			     if(RT_METHOD==2)then !ADD IN THE C AVAILABLE, and THE NDEMAND TO ACQUIRE FROM THE FALLING LEAVES. 
			      !npp_remaining(p,istp)          =  npp_remaining(p,istp) + storage_cdemand(p) * permyc(p,istp) 
			      ! We don't actually have any more carbon at the offset timestep to use, unless we use some stored C?
			      ! In principle, there is just a cheaper source of N. We could use some stored carbon to pay for it? 
			      plant_ndemand_pool_step(p,istp)= plant_ndemand_pool_step(p,istp) &
                                                + plant_ndemand_retrans(p)   * permyc(p,istp)
			     endif
			     
			     if(use_flexiblecn)then   
			     ! calculate actual leaf CN ratio
                if (leafn(p) == 0.0_r8) then   ! to avoid division by zero, and also to make leafcn_actual(p) a very large number if leafn(p) is zero
                  actual_leafcn = leafc(p) / 0.000000001_r8
                else
                  actual_leafcn = (leafc(p)+leafc_storage(p))/(leafn(p)+leafn_storage(p)) ! leaf CN ratio
                end if
                delta_CN = actual_leafcn - leafcn(ivt(p))
              end if !vary plant CN.
			     
              lloop: do j = 1,nlevdecomp
	                !Calculate the hypothetical amount of NPP that we should use to extract N in this layer
	                ...
	                if(cost_minimized)then
                     npp_frac_to_fixation(j) =0.0_r8
                     npp_frac_to_retrans(j) =0.0_r8
                     npp_frac_to_active_nh4(j) =0.0_r8
                     npp_frac_to_active_no3(j) =0.0_r8
                     npp_frac_to_nonmyc_nh4(j) =0.0_r8
                     npp_frac_to_nonmyc_no3(j) =0.0_r8
                     mincost = min(cost_fix(p,j),cost_retran(p,j),cost_active_nh4(p,j), &
                                   cost_nonmyc_nh4(p,j),cost_active_no3(p,j),cost_nonmyc_no3(p,j))
                     if(cost_fix(p,j).eq. mincost)then
                       npp_frac_to_fixation(j) = 1.0_r8
                     end if
                     if(cost_retran(p,j) .eq. mincost)then
                       npp_frac_to_retrans(j) = 1.0_r8
                     end if
                     if(cost_active_no3(p,j) .eq. mincost)then
                       npp_frac_to_active_no3(j) = 1.0_r8
                     end if
                     if(cost_active_nh4(p,j) .eq. mincost)then
                       npp_frac_to_active_nh4(j) = 1.0_r8
                     end if
                     if(cost_nonmyc_no3(p,j) .eq. mincost)then
                       npp_frac_to_nonmyc_no3(j) = 1.0_r8
                     end if
                     if(cost_nonmyc_nh4(p,j) .eq. mincost)then
                       npp_frac_to_nonmyc_nh4(j) = 1.0_r8
                     end if
                     cost_resis(p,j) = mincost
                   endif !cost minimized
                   
                   if(use_flexiblecn)then 
                     ! C used for uptake is reduced if the cost of N is very high                
                     frac_ideal_C_use = max(0.0_r8,1.0_r8 - (cost_resis(p,j)-5.0_r8)/200.0_r8 )
                     ...
                     if(delta_CN .gt.0.and. frac_ideal_C_use.lt.1.0)then           
                       frac_ideal_C_use = frac_ideal_C_use + (1.0_r8-frac_ideal_C_use)*min(1.0_r8, delta_CN/80._r8);                       
                     end if    
                     frac_ideal_C_use = max(min(1.0_r8,frac_ideal_C_use),0.5_r8) ! don't let this go above 1 or below an arbirtray minimum (to prevent zero N uptake). 
                   else
                   ...
                   excess_carbon                 = npp_remaining(p,istp)*(1.0_r8-frac_ideal_c_use)
                   if(excess_carbon*(1.0_r8+grperc(ivt(p))).gt.npp_remaining(p,istp))then !prevent negative dnpp
                      excess_carbon =  npp_remaining(p,istp)/(1.0_r8+grperc(ivt(p)))
                   endif
                   
                   npp_remaining(p,istp)         = npp_remaining(p,istp)-excess_carbon*(1.0_r8+grperc(ivt(p)))
                  ! Ho
                   dnpp  = npp_remaining(p,istp) / ( (1.0_r8+grperc(ivt(p)))*(plantCN(p) / cost_resis(p,j)) + 1._r8)  
                   dnpp  = dnpp * frac_ideal_C_use
                   ! Force uptake into lower layers, to avoid unrealistic depletion of top layer
                   ! In another format, one could consider the layers in ascending order of cost...
                   dnpp  = min(dnpp, availc_pool(p)*rootfr(p,j))
	              	                
	                !hypothetical amount of N acquired. 
	                dn    = dnpp / cost_resis(p,j)
               
 	                
	                ! How much of this NPP carbon do we allocate to the different pathways? fraction x gC/m2/s rosief
	               npp_to_fixation(j)   = npp_frac_to_fixation(j)   * dNPP
	               npp_to_retrans(j)    = npp_frac_to_retrans(j)    * dNPP
	               ...
	               n_from_retrans(j)    = npp_to_retrans(j)     / cost_retran(p,j)
	               ...
	               !if(abs(npp_to_fixation(j)+npp_to_retrans(j)+npp_to_active_nh4(j) &
                       !      +npp_to_nonmyc_nh4(j)+npp_to_active_no3(j)+npp_to_nonmyc_no3(j)-dnpp)&
	               !     .gt.0.000000000001_r8)then
	               !     write(iulog,*) 'npp allocation terms off'                     
	               !endif  !check 
	               	                  
	               !if(abs(n_from_fixation(j)+n_from_retrans(j)+n_from_active_nh4(j) &
                       !       +n_from_nonmyc_nh4(j)+n_from_active_no3(j)+n_from_nonmyc_no3(j)-dn)&
	               !     .gt.0.000000000001_r8)then
	               !     write(iulog,*) 'N allocation terms off'                     
	               !endif  !check

                    
                  ! --------------------RETRANSLOCATION UPTAKE LIMIT------------------------!                             
                  ! Limit to the amount we can retranslocate.  ! what are these two different fluxes?                    
                  retrans_limit1  =  min(litterfall_step(p,istp),leafn_step(p,istp))           

                  if (n_retrans_acc(p,istp) + n_from_retrans(j) - retrans_limit1.ge.0.0000000000001_r8) then                
                     n_from_retrans(j)       = retrans_limit1 - n_retrans_acc(p,istp)
                     ! how much did it cost?
                  end if      
                  npp_to_retrans(j)       = n_from_retrans(j) * cost_retran(p,j)
                  ! Accumulate n_retrans over soil layers.            
                  
                 ! --------------------ACTIVE UPTAKE NO3 UPTAKE LIMIT------------------------!  
                  active_no3_limit1          = sminn_no3_layer_step(p,j,istp)
                  if (n_from_active_no3(j) + n_from_nonmyc_no3(j).gt.active_no3_limit1) then ! trying to remove too much nh4 from soil. 
                  ...
                  active_nh4_limit1          = sminn_nh4_layer_step(p,j,istp)
                  if (n_from_active_nh4(j) + n_from_nonmyc_nh4(j).gt.active_nh4_limit1) then ! trying to remove too much nh4 from soil. 
                  ...
                  !if(n_from_nonmyc_nh4(j).lt.-0.000000001_r8.or.n_from_nonmyc_no3(j).lt.-0.000000001_r8)then
                     !write(iulog,*) 'negative nuptake from soil',n_from_nonmyc_no3(j),n_from_nonmyc_nh4(j)
                  !endif
                                                 
                  ! How much N did we end up with
                  N_acquired                    = n_from_fixation(j) + n_from_retrans(j) &
                                                  + n_from_active_no3(j)+n_from_nonmyc_no3(j) &
                                                  + n_from_active_nh4(j)+n_from_nonmyc_nh4(j)
                  ! How much did it actually cost? 
                  C_spent                       = npp_to_fixation(j) + npp_to_retrans(j) &
                                                  + npp_to_active_no3(j)+npp_to_nonmyc_no3(j) &
                                                  + npp_to_active_nh4(j)+npp_to_nonmyc_nh4(j)
                  
                  excess_carbon_acc             = excess_carbon_acc + excess_carbon

                  ! How much C do we have left to allocate in the next soil layer?
                  npp_remaining(p,istp)         = npp_remaining(p,istp)   - C_spent - (N_acquired &
                  ...
                  n_retrans_acc(p,istp)         = n_retrans_acc(p,istp)    + n_from_retrans(j)
                  ...
                  npp_retrans_acc(p,istp)    = npp_retrans_acc(p,istp)     + npp_to_retrans(j)
                  
                 
                                       
                  if (npp_remaining(p,istp).le.0.0000000000001_r8)then
                    exit lloop 
                  end if
             ...
             if (npp_remaining(p,istp).ge.0.0000000000001_r8)then
                  burned_off_carbon =  burned_off_carbon+npp_remaining(p,istp)
             end if
         ...
         ! add vertical fluxes to patch arrays 
            do j = 1,nlevdecomp
               ...
               n_fix_vr(p,j)             =  n_fix_vr(p,j)             + n_from_fixation(j)
            end do
...
!---------------------------N fluxes--------------------!
      ...
      Nretrans(p)               = n_retrans_acc_total(p)/dt 
      retransn_to_npool(p)      = Nretrans(p)
      
      !if(Nretrans(p).gt.leafn(p)/dt)then 
      !   write(iulog,*) 'too much N extracted from leaves',p,Nretrans(p),leafn(p)/dt
      !endif     
      ...
      Nactive(p)                = Nactive_no3(p)  + Nactive_nh4(p)
      ...
      Nuptake(p)                = Nactive_no3(p) + Nactive_nh4(p) + Nnonmyc_no3(p) &
                                  + Nnonmyc_nh4(p) + Nfix(p) + Npassive(p) + Nretrans(p)
...
!---------------------------Extra Respiration Fluxes--------------------!      
      soilc_change(p)           = (npp_active_no3_acc_total(p)      + npp_active_nh4_acc_total(p) &
                                    ...
                                    + npp_nonmyc_nh4_acc_total(p)     + npp_fix_acc_total(p)      &
                                    + npp_retrans_acc_total(p))/dt 
      soilc_change(p)           = soilc_change(p) + burned_off_carbon / dt                 
                 
      npp_Nuptake(p)            = soilc_change(p)
      ! how much carbon goes to growth of tissues?  
      npp_growth(p)             = Nuptake(p)*plantCN(p)+(excess_carbon_acc/dt) !does not include gresp, since this is calculated from growth
       carbon_for_allocation = (excess_carbon_acc+Nuptake(p)*dt*plantCN(p))*(1.0_r8+grperc(ivt(p)))

      !if(npp_growth(p).gt.0.001_r8)then
        !write(iulog,*) 'npp_growth',npp_growth(p)*dt,p
        !write(iulog,*) 'excess_carbon_acc',excess_carbon_acc
        !write(iulog,*) 'nuptake',Nuptake(p)
        !write(iulog,*) 'plantcn',plantCN(p)
      !endif
      
!---------------------------Check Carbon Balance--------------------! 
     carbon_for_allocation = Nuptake(p)*dt*plantCN(p)*(1.0_r8+grperc(ivt(p)))+excess_carbon_acc
...
!===========================================================================
  real(r8) function fun_cost_retrans(big_cost,kr_resorb,leaf_narea,smallValue)
...
!--------------------------------------------------------------------------
  ...
  real(r8), intent(in) :: leaf_narea       !  N stored in leaves that are about to fall off (gN/m2)
  real(r8), intent(in) :: smallValue       !  a small number
 
  if (leaf_narea > smallValue) then
    fun_cost_retrans = kr_resorb / leaf_narea
  else
