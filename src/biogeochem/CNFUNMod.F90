...
! !PUBLIC MEMBER FUNCTIONS:
  ...
  integer, parameter :: COST_METHOD = 2 !new way of doing the N uptake
...
!--------------------------------------------------------------------
   !-----------
  logical, parameter             :: l_fun_fix     = .TRUE.          ! TRUE if
  !  fix             is allowed
  logical, parameter             :: l_fun_passive = .FALSE.          ! TRUE if
  !  passive uptake  is allowed
  logical, parameter             :: l_fun_active  = .TRUE.          ! TRUE if
  !  active uptake   is allowed
  logical, parameter             :: l_fun_retrans = .TRUE.          ! TRUE if
  !  retranslocation is allowed
  logical, parameter             :: l_fun_nonmyc  = .TRUE.          ! TRUE if
  !  non-mycorrhizal is allowed
! -------------------------------------------------------------------
  ! ------------
! Inteter parameters
!--------------------------------------------------------------------
  !-----------
  integer,  parameter :: nstp            = 2             ! Number of
  !  calculation part
...
!if (.not. use_nitrif_denitrif) then
  integer,  parameter :: ncost4          = 4             ! Number of
  !  N transport pathways
  integer,  parameter :: icostActiveSmin = 3             ! Process
  !  number for mycorrhizal uptake.
  integer,  parameter :: icostNonmyc     = 4             ! Process
  !  number for nonmyc uptake.
!else  !----------NITRIF_DENITRIF-------------! 
  integer,  parameter :: ncost6          = 6             ! Number of
  !  N transport pathways
...
!end if  !end of if_not_use_nitrif_denitrif
  real(r8), parameter :: big_cost        = 1000000000._r8! An
...
!--------------------------------------------------------------------
  ...
  associate(ivt                    => patch%itype                                          , & ! Input:   [integer  (:) ]  p
         ...
         !  parameter
         kr_resorb              => pftcon%kr_resorb                                     , & ! Input:   A retranslocation
         !  parameter
...
!--------------------------------------------------------------------
            ...
            if (plant_ndemand_pool_step(p,istp) <= n_passive_step(p,istp)) then 
                ...
                do j = 1,  nlevdecomp
                      sminn_no3_uptake(p,j,istp)   = sminn_no3_uptake(p,j,istp) - (excess/dt) * &
                      sminn_no3_uptake(p,j,istp)/ (n_passive_step(p,istp)/dt)
                      sminn_nh4_uptake(p,j,istp)   = sminn_nh4_uptake(p,j,istp) - (excess/dt) * &
                      sminn_nh4_uptake(p,j,istp)/ (n_passive_step(p,istp)/dt)
                end do  ! j 
                n_passive_step(p,istp)             = plant_ndemand_pool_step(p,istp)
            end if  !  n_demand_step <= n_passive_step
            
            if (plant_ndemand_pool_step(p,istp) ==0.0_r8) then 
                unmetDemand = .FALSE.
            end if
...
!--------------------------------------------------------------------
            ...
            n_passive_acc(p)                   = n_passive_acc(p)                + n_passive_step(p,istp)    ! step
            ! -level accmulation
...
!--------------------------------------------------------------------
            !------------
           if (unmetDemand.and.plant_ndemand_pool_step(p,istp)>0.0_r8.and.availc(p).gt.0.0_r8) then 
!--------------------------------------------------------------------
              !------------
         ...
         !------Retranslocation Uptake Cost-------------!  
            if (l_fun_retrans) then
               ...
               if (season_decid(ivt(p)) == 1._r8 .or. stress_decid(ivt(p)) == 1._r8) then
                  ...
                  if (offset_flag(p) == 1._r8) then
                  ! for deciduous trees, the whole leafn pool is
                     !  available for retranslocation. 
                     costNit(:,icostRetrans) = fun_cost_retrans(big_cost,kr_resorb(ivt(p)),leaf_narea,smallValue)
                  else
                     costNit(:,icostRetrans) = big_cost 
                  end if
               else
                  ! for evergreen trees, only part of the leafn pool
                  !  is available for retranslocation. 
                  costNit(:,icostRetrans)    = fun_cost_retrans(big_cost,kr_resorb(ivt(p)),leaf_narea,smallValue)
               end if
            else
               costNit(:,icostRetrans)       = big_cost
           ...
           ! costNit(:,icostRetrans)       = big_cost ! add this in
            !  as a test RF. REMOVE
           ...
           ! Remove C required to pair with N from passive uptake
            ...
            do FIX =1,2 !loop around percentages of fixers and non
               ! fixers, with differnt costs. 
            if(FIX==1)then ! How much of the carbon in this PFT can in principle be used for fixation? 
      ...
      !----------- 
           ...
           total_N_conductance = 0.0_r8
            do j = 1, nlevdecomp
               ...
               total_N_conductance  = total_N_conductance + 1._r8/cost_active_no3(p,j) + 1._r8/cost_active_nh4(p,j) &
                              + 1._r8/cost_nonmyc_no3(p,j)      + 1._r8/cost_nonmyc_nh4(p,j) & 
                              + 1._r8/cost_retran(p,j)
               if(FIX==1)then
                  total_N_conductance  = total_N_conductance + cost_res_resis(p,j) + 1._r8/cost_fix(p,j) 
               end if 
             ...
             do j = 1, nlevdecomp     
               ...
               npp_frac_to_retrans(j)    = (1._r8/cost_retran(p,j))     / total_N_conductance
               ...
               if(FIX==1)then
                 npp_frac_to_fixation(j)   = (1._r8/cost_fix(p,j))        / total_N_conductance
               else
               ...
               sum_n_acquired        =  sum_n_acquired + n_exch_retrans(j) + n_exch_active_nh4(j) +&
            ...
            if(local_use_flexiblecn)then   
              ...
              do j = 1,nlevdecomp
               ...
               ! did we exceed the limits of uptake for any of these pools?
                  ...
                  retrans_limit1  =  min(litterfall_step(p,istp)*fixerfrac,leafn_step(p,istp)*fixerfrac)           
               ...
               ! Remove this term as now we always go through the whole loop.                         
                !  if (npp_to_spend.le.0.0000000000001_r8)then
                !    exit lloop 
                !  end if
