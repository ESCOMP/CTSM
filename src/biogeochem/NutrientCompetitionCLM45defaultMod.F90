...
implicit none
  ...
  type, extends(nutrient_competition_method_type) :: &
       nutrient_competition_clm45default_type
     private
   contains
     procedure :: init_nutrient_competition           ! initialize nutrient competition mod
     procedure :: read_nutrient_competition_params    ! read nutrient competition parameters
     procedure :: calc_nutrient_competition           ! calculate nutrient yield rate from competition
     procedure :: calc_plant_nutrient_demand          ! calculate plant nutrient demand 
...
contains
  ...
  !-----------------------------------------------------------------------  
  subroutine init_nutrient_competition(this, bounds, crop_prog)
  
  !
  ! DESCRIPTION
  ! initialize the NutrientCompetition_clm45default_Mod
  !
  ! USES
  ...
  !allocate array for crop model when it is needed    
  if ( crop_prog )then
  ...
  end subroutine init_nutrient_competition
  !-----------------------------------------------------------------------
  
  subroutine read_nutrient_competition_params (this, ncid )
    !
    ! DESCRIPTION
    ! read parameters from namelist for nitrogen competition algorithm setup
    !
    ! !USES:
    ...
    NutrientCompetitionParamsInst%bdnr=tempr

    tString='dayscrecover'
    call ncd_io(varname=trim(tString),data=tempr, flag='read', ncid=ncid, readvar=readv)
    if ( .not. readv ) call endrun(msg=trim(errCode)//trim(tString)//errMsg(__FILE__, __LINE__))
    NutrientCompetitionParamsInst%dayscrecover=tempr
  ...
  end subroutine read_nutrient_competition_params

  !-----------------------------------------------------------------------  
  subroutine calc_nutrient_competition(this, bounds, num_soilc, filter_soilc, num_soilp, filter_soilp, &
  ...
  !
  ! DESCRIPTION
  ! do nutrient competition using the default clm45 method
  !
  class(nutrient_competition_clm45default_type), intent(in) :: this
  ...
  call nitrogen_competition(bounds, num_soilc, filter_soilc, cnstate_vars, nitrogenstate_vars, nitrogenflux_vars)
  ...
  end subroutine calc_nutrient_competition
...
!-------------------------------------------------------------------------------
  ...
  subroutine nitrogen_competition(bounds, num_soilc, filter_soilc, cnstate_vars, nitrogenstate_vars, nitrogenflux_vars)
  !
  ! DESCRIPTION
  ! do nitrogen competition using the default approach as implmented in clm45.
  !
  ! !USES:
  use clm_varctl       , only: cnallocate_carbon_only, use_nitrif_denitrif    
  use clm_varpar       , only: nlevsoi, nlevdecomp
  use clm_varcon       , only: nitrif_n2o_loss_frac, secspday  
  use clm_time_manager , only: get_step_size
  use shr_infnan_mod  , only: nan => shr_infnan_nan, assignment(=)  
  ...
  bdnr         = NutrientCompetitionParamsInst%bdnr * (dt/secspday)
  dayscrecover = NutrientCompetitionParamsInst%dayscrecover  
  !not sure if I have to updated sminn_vr here, Jinyun Tang, Sep 9, 2014
  !calcualte nitrogen uptake profile
  !nuptake_prof(:,:) = nan
  !call calc_nuptake_prof(bounds, nlevdecomp, num_soilc, filter_soilc, sminn_vr, &
  !       dzsoi_decomp, nfixation_prof, nuptake_prof)
...
!!!         
  ...
  end subroutine nitrogen_competition

!------------------------------------------------------------------------------------
  subroutine calc_plant_nutrient_demand(this, bounds, num_soilc, filter_soilc, num_soilp, filter_soilp, &
  ...
  !
  ! DESCRIPTION
  ! calculate plant nutrient demand
  !
    class(nutrient_competition_clm45default_type), intent(in) :: this
    ! !ARGUMENTS:
    type(bounds_type)        , intent(in)    :: bounds
