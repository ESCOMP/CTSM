...
sub process_namelist_inline_logic {
  ...
  setup_logic_hillslope($opts->{'test'}, $nl_flags, $definition, $defaults, $nl, $physv);
...
sub setup_logic_hillslope {
  ...
  if ( $physv->as_long() >= $physv->as_long("clm4_5") ) {
    add_default($test_files, $nl_flags->{'inputdata_rootdir'}, $definition, $defaults, $nl, 'use_hillslope' );
  }
}
