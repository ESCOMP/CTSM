...
Constants :math:`b` and :math:`{f}_{lu}` are calibrated based on observations and reanalysis datasets in the Amazon rainforest (tropical closed forests within 15.5 :sup:`o` S :math:`\text{-}` 10.5 :sup:`o` N, 30.5 :sup:`o` W :math:`\text{-}` 91 :sup:`o` W). :math:`b` = 0.033 d\ :sup:`-1` and :math:`f_{lu}` is defined as
