...
def main ():
    ...
    fsurdat_in = get_config_value(config, 'modify_input',
                                  'fsurdat_in', cfg_path)
    fsurdat_out = get_config_value(config, 'modify_input',
                                   'fsurdat_out', cfg_path)
    idealized = bool(get_config_value(config, 'modify_input',
                                      'idealized', cfg_path))

    # not required: user may set these in ./modify.cfg
    # TODO add error checks to confirm validity of entries?
    ...
    dom_nat_pft = select_value(var=temp, default=0, type_of_var=int)

    temp = get_config_value(config, 'modify_input', 'lai', cfg_path)
    lai = select_value(var=temp, default=[0]*12, type_of_var=float)

    temp = get_config_value(config, 'modify_input', 'sai', cfg_path)
    sai = select_value(var=temp, default=[0]*12, type_of_var=float)

    temp = get_config_value(config, 'modify_input', 'hgt_top', cfg_path)
    hgt_top = select_value(var=temp, default=[0]*12, type_of_var=float)

    temp = get_config_value(config, 'modify_input', 'hgt_bot', cfg_path)
    hgt_bot = select_value(var=temp, default=[0]*12, type_of_var=float)
