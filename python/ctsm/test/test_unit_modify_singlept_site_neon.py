...
from ctsm.site_and_regional.modify_singlept_site_neon import get_parser, get_neon, find_surffile, find_soil_structure, update_metadata, update_time_tag, sort_print_soil_layers, check_neon_time, download_file, fill_interpolate
...
class TestModifySingleptSiteNeon(unittest.TestCase):
    ...
    def test_update_time_tag(self):
        ...
        """
        self.assertEqual(update_time_tag("test_YYMMDD.nc")[-9:-3], date.today().strftime("%y%m%d"), "File ending not as expected")

    def test_check_neon_time(self):
        """
        Test that dictionary containing last modified information is correctly downloaded
        """
        last_abby_download = check_neon_time()['https://storage.neonscience.org/neon-ncar/NEON/surf_files/v1/ABBY_surfaceData.csv']
        self.assertEqual(len(last_abby_download), 19, "last ABBY download has unexpected date format or does not exist")
        ...
        self.assertGreater(int(last_abby_download[:4]), 2021, "ABBY download is older than expected")
