...
class TestLongitude:
    ...
    @pytest.mark.parametrize("lon", [-180, -55, 55, 155, 180])
    def test_check_lon_type_180(self, lon):
        """
        Check that a single value in [-180, 180] passes _check_lon_type_180()
        """
        _check_lon_type_180(lon)
    ...
    def test_check_lon_type_180_list_errors(self):
        ...
        """
        with pytest.raises(ValueError, match=r"lon_in must be in the range \[-180, 180\]"):
            _check_lon_type_180([-191, -180, -55, 55, 155, 180])

    @pytest.mark.parametrize("lon", [0, 55, 180, 180])
    def test_check_lon_type_360(self, lon):
        """
        Check that a single value in [0, 360] passes _check_lon_type_360()
        """
        _check_lon_type_360(lon)
    ...
    def test_check_lon_type_360_list_errors(self):
        ...
        """
        with pytest.raises(ValueError, match=r"lon_in must be in the range \[0, 360\]"):
    ...
    def test_convert_lon_type_180_to_360(self, lon_in, lon_exp):
        """Test conversion 180→360"""
        lon_out = _convert_lon_type_180_to_360(lon_in)
        assert lon_out == lon_exp
    ...
    @pytest.mark.parametrize("this_lon", [-55, 87, -180, 180])
    def test_lon_obj_type180(self, this_lon):
        """Test that creating an in-bounds Longitude of type 180 works"""
        lon_type = 180
        lon_obj = Longitude(this_lon, lon_type)
        assert lon_obj.get(lon_type) == this_lon

    @pytest.mark.parametrize("this_lon", [87, 0, 360])
    def test_lon_obj_type360_pos(self, this_lon):
    ...
    def test_lon_eq_360num_error(self):
        ...
        lon2 = 275
        with pytest.raises(
            TypeError, match="Comparison not supported between instances of 'Longitude' and "
        ):
            _ = lon1 == lon2
    ...
    def test_detect_lon_type_list_both(self):
        ...
        with pytest.raises(RuntimeError, match=r"Longitude array contains values of both types"):
    ...
    def test_detect_lon_type_oob_low(self):
        ...
        with pytest.raises(ValueError, match=r"\(Minimum\) longitude < -180"):
            detect_lon_type(-300)

    def test_detect_lon_type_oob_high(self):
        """test that detect_lon_type fails if out of bounds above max"""
        with pytest.raises(ValueError, match=r"\(Maximum\) longitude > 360"):
            detect_lon_type(500)
    ...
    def test_list_as_lon(self):
        ...
        assert expected_lon == result._lon
        assert expected_type == result.lon_type()
    ...
    def test_no_implicit_string_conversion(self):
        ...
        with pytest.raises(
            NotImplementedError,
            match=r"Use Longitude\.get_str\(\) instead of implicit string conversion",
        ):
            _ = f"{lon}"
        with pytest.raises(
            NotImplementedError,
    ...
    def test_get_str(self):
        ...
        lon = Longitude(55, 180)
        assert lon.get_str(180) == "55.0"
        assert lon.get_str(360) == "55.0"
        lon = Longitude(-55, 180)
        assert lon.get_str(180) == "-55.0"
        assert lon.get_str(360) == "305.0"
