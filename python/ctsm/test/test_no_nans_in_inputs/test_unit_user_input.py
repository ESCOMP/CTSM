...
class TestGetFillValueFromUser:
    ...
    def test_integer_input(self, monkeypatch):
        ...
        expected = 42
        monkeypatch.setattr("builtins.input", lambda _: str(expected))
        result = _get_fill_value_from_user(INT_VAR_CONTEXT, FillValueConfig())
        assert result == expected
    ...
    def test_delete_command(self, monkeypatch):
        ...
        result = _get_fill_value_from_user(DEFAULT_VAR_CONTEXT, FillValueConfig(allow_delete=True))
    ...
    def test_delete_not_allowed(self, monkeypatch):
        ...
        result = _get_fill_value_from_user(DEFAULT_VAR_CONTEXT, FillValueConfig(allow_delete=False))
    ...
    def test_skip_variable(self, monkeypatch):
        ...
        with pytest.raises(ValueError, match=ERR_STR_SKIP_VAR):
            _get_fill_value_from_user(DEFAULT_VAR_CONTEXT, FillValueConfig())
    ...
    def test_ctrl_c_then_valid_input(self, monkeypatch):
        ...
        def input_with_ctrl_c(_):
            ...
            call_count += 1
            if call_count == 1:
                # First call: simulate Ctrl-C. _get_fill_value_from_user will
