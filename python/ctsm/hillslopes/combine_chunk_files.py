...
if addStream:
    osdepth = w.createVariable('hillslope_stream_depth',np.float64,('lsmlat','lsmlon',))
    osdepth.units = 'meters'
    osdepth.long_name = 'stream channel bankfull depth'
    oswidth = w.createVariable('hillslope_stream_width',np.float64,('lsmlat','lsmlon',))
    oswidth.units = 'meters'
    oswidth.long_name = 'stream channel bankfull width'
    osslope = w.createVariable('hillslope_stream_slope',np.float64,('lsmlat','lsmlon',))
    osslope.units = 'm/m'
...
if addBedrock:
    obed[:,] = h_bedrock
if addStream:
    osdepth[:,] = h_stream_depth
    oswidth[:,] = h_stream_width
    osslope[:,] = h_stream_slope
