...
cfile = cdir + 'chunk_{:02d}_HAND_4_col_hillslope_geo_params_section_quad_{}.nc'.format(cndx,dem_source)
...
command = 'ls '+gfile
gfiles = subprocess.run(command,capture_output=True,shell='True').stdout.decode('utf-8').split('\n')[:-1]
...
for gfile in gfiles:
    ...
    h_elev = np.asarray(f.variables['h_height'][:,])
    h_dist = np.asarray(f.variables['h_length'][:,])
    h_width = np.asarray(f.variables['h_width'][:,])
    h_area = np.asarray(f.variables['h_area'][:,])
    h_slope = np.asarray(f.variables['h_slope'][:,])
    h_aspect = np.asarray(f.variables['h_aspect'][:,])
    if addBedrock:
        h_bedrock = np.asarray(f.variables['h_bedrock'][:,])
    if addStreamChannelVariables:
        h_stream_depth = np.asarray(f.variables['h_stream_depth'][:,])
        h_stream_width = np.asarray(f.variables['h_stream_width'][:,])
        h_stream_slope = np.asarray(f.variables['h_stream_slope'][:,])


        

    nhillcolumns = f.variables['nhillcolumns'][:,].astype(int)
    pct_hillslope = f.variables['pct_hillslope'][:,]
    hillslope_index = f.variables['hillslope_index'][:,].astype(int)
    column_index = f.variables['column_index'][:,].astype(int)
    downhill_column_index = f.variables['downhill_column_index'][:,].astype(int)
    f.close()
